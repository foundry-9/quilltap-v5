...
/// steps below it.
///
/// 1. Concierge mode OFF → unchanged.
/// 2. Chat marked dangerous AND the scene routes uncensored → unchanged.
/// 3. Classify the concatenated appearance text; safe → unchanged.
/// 4. Dangerous AND the scene routes uncensored → unchanged (it goes to a
///    provider that accepts it, so accuracy beats a draped sheet).
/// 5. Otherwise → sanitize via the cheap LLM.
///
/// [cc65d6bfc] `routes_dangerous_to_uncensored` is deliberately NOT "an
/// uncensored profile is configured" (bug 133). Whether one exists in settings
/// and whether *this* scene will actually be generated by it are different
    ...
    C: CompletionProvider,
{
    ...
    if is_dangerous_chat && routes_dangerous_to_uncensored {
        return appearances;
    }
    ...
    if routes_dangerous_to_uncensored {
        return appearances;
    }
