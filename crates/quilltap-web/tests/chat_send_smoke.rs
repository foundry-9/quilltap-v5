...
use quilltap_host::spine::{ChatSpine, SpineBundle, SpineFactory};
...
impl SpineFactory for SmokeSpineFactory {
    fn build(
        ...
        let spine = Arc::new(ChatSpine {
            db: db.clone(),
            events: events.clone(),
            embedding: Arc::new(CannedEmbeddingProvider::new()),
            completion: Arc::new(AnyCompletion),
            streaming: Arc::new(AnyStream),
