        ...
        let saved: Value = rt
            .block_on(db.write(move |writers| {
                ...
                let r = quilltap_core::photos::character_gallery_service::save_link_to_character_gallery(
                    main, mount, ARIA, &source_link, None, &[], FIXED_KEPT_AT, &quilltap_host::HostImageCodec,
                );
                    ...
                    Ok(parsed) => {
                        ...
                        let saved: Value = rt
                            .block_on(db.write(move |writers| {
                                ...
                                let main = writers.main().connection();
                                let r = quilltap_core::photos::character_gallery_service::save_link_to_character_gallery(
                                    main, mount, ARIA, &link, caption.as_deref(), &tags, FIXED_KEPT_AT, &quilltap_host::HostImageCodec,
                                );
                                Ok(r.map_err(|e| format!("{e:?}")))
                            }))
                            .unwrap()
