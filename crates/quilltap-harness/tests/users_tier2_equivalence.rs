//! Tier-2 differential test: the `users` repo (Phase-2).
//!
//! Structural DB diff. Both sides start from the SAME seed fixture (built by
//! harness/oracle/fixtures/build-users-fixture.ts), run the SAME create / update
//! / delete op sequence from the committed spec, dump the `users` table
...
//!
//! This banks the plainest marshaling surface so far: one required TEXT column
//! (`username`) plus five nullable TEXT columns (`email`, `name`, `image`,
//! `emailVerified`, `passwordHash`) — `None` -> SQL NULL, `Some` -> the string.
//!
...
//! Run:
//!   QT_ORACLE_USERS=/tmp/oracle-us.ndjson \
//!   QT_FIXTURE_USERS=/tmp/qt-us-fixture.db \
//!     cargo test -p quilltap-harness --test users_tier2_equivalence

use std::path::{Path, PathBuf};

use quilltap_core::db::users::{CreateOptions, UserCreate, UserUpdate};
...
fn spec_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../harness/oracle/fixtures/users-tier2.json")
}

#[test]
fn users_tier2_matches_oracle() {
    let oracle_path = match std::env::var("QT_ORACLE_USERS") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_ORACLE_USERS to the oracle NDJSON (see test header).");
            return;
        }
    };
    let fixture = match std::env::var("QT_FIXTURE_USERS") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_FIXTURE_USERS to the seed fixture .db (see test header).");
            return;
        }
    };
        ...
        .unwrap_or_else(|e| panic!("open fixture copy: {e}"));
    {
        ...
        for op in &spec.ops {
            match op {
                ...
                Op::Delete { id } => {
                    let found = repo.delete(id).expect("users.delete");
    ...
    eprintln!("OK: users tier-2 matched oracle ({n} rows).");
}
