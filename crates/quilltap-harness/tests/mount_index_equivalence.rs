...
//! sides' RAW eight-table dumps (+ response bodies) are normalized HERE, by one
//! shared algorithm: timestamps blanked, minted UUIDs remapped in a canonical
//! traversal order, extraction-error text collapsed (v4 pdf-parse vs the v5
//! refusing seam), REAL-affinity numbers canonicalized.
//!
...
fn rows_to_json(conn: &rusqlite::Connection, sql: &str) -> Vec<Value> {
...
fn norm(case: &Value) -> String {
    serde_json::to_string_pretty(&sorted(&normalize(case))).unwrap()
}

fn diff_first_line(got: &str, want: &str) -> String {
...
fn status_of_kind(kind: &ErrorKind) -> i64 {
    match kind {
...
fn response_to_status_body(r: Response) -> (i64, Value) {
    match r {
        Response::MountFile(v) => (200, v),
        Response::Error(e) => {
            let status = status_of_kind(&e.kind);
            // v4's error bodies: `{error}` (or `{error, code}` for file-op
            // failures — the code rides the message as "msg\u{0}code"? No:
            // handlers in this family emit plain `{error}`; scan/reindex/embed
            // 404s and 500s carry no code).
...
fn mount_index_matches_oracle() {
    let path = match std::env::var("QT_ORACLE_MOUNT_INDEX") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_ORACLE_MOUNT_INDEX to the oracle NDJSON (see test header).");
            return;
        }
    };
