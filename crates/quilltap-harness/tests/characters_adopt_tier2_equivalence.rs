//! Tier-2 differential test: the ADOPT branch of v4's `ensureCharacterVault`
//! (Phase-2, the store-backed capstone).
//!
//! Proves [`character_vault::ensure_character_vault`]'s startup-heal path: a
//! vault-less character with a populated same-name orphan store, receiving a
//! managed-field update, ADOPTS the existing store (linking it, no duplicate mint)
//! rather than provisioning fresh. Reached through the public API:
//! `update_character` → the no-vault-but-managed-field branch →
//! `ensure_character_vault` → the adopt search (exactly one populated same-name
//! `character` store qualifies → adopt).
//!
//! Both sides start from the SAME baked fixture (a character + populated vault
...
//!   QT_FIXTURE_CHARADOPT_MAIN=/tmp/qt-charadopt-main.db \
//!   QT_FIXTURE_CHARADOPT_MOUNT=/tmp/qt-charadopt-mount.db \
//!     $N/npx tsx ~/source/quilltap-v5/harness/oracle/cases/characters-adopt.ts > /tmp/oracle-charadopt.ndjson
//! Run:
//!   QT_ORACLE_CHARADOPT=/tmp/oracle-charadopt.ndjson \
//!   QT_FIXTURE_CHARADOPT_MAIN=/tmp/qt-charadopt-main.db \
//!   QT_FIXTURE_CHARADOPT_MOUNT=/tmp/qt-charadopt-mount.db \
//!     cargo test -p quilltap-harness --test characters_adopt_tier2_equivalence
...
fn spec_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../harness/oracle/fixtures/characters-adopt-tier2.json")
}
...
#[test]
fn characters_adopt_tier2_matches_oracle() {
    let oracle_path = match std::env::var("QT_ORACLE_CHARADOPT") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_ORACLE_CHARADOPT to the oracle NDJSON (see header).");
            return;
        }
    };
    let main_fixture = match std::env::var("QT_FIXTURE_CHARADOPT_MAIN") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_FIXTURE_CHARADOPT_MAIN to the main fixture .db (header).");
            return;
        }
    };
    let mount_fixture = match std::env::var("QT_FIXTURE_CHARADOPT_MOUNT") {
        Ok(p) => p,
        Err(_) => {
            eprintln!("SKIP: set QT_FIXTURE_CHARADOPT_MOUNT to the mount fixture .db (header).");
            return;
        }
    };
    ...
    let main_work = std::env::temp_dir().join(format!("qt-charadopt-main-rust-{pid}.db"));
    let mount_work = std::env::temp_dir().join(format!("qt-charadopt-mount-rust-{pid}.db"));
    ...
    // Precondition: the character starts vault-less but a populated same-name store
    // survives (exactly one mount point in the fixture — the adopt candidate).
    ...
    let mp_before: i64 = mount
        .connection()
        .query_row("SELECT COUNT(*) FROM doc_mount_points", [], |row| {
            row.get(0)
        })
        .expect("count mount points");
    assert_eq!(
        mp_before, 1,
        "fixture precondition: exactly one populated same-name store to adopt"
    );
    ...
    // The adopt must NOT have minted a second mount point.
    let mp_after: i64 = mount
        .connection()
        .query_row("SELECT COUNT(*) FROM doc_mount_points", [], |row| {
            row.get(0)
        })
    ...
    assert_eq!(
        mp_after, 1,
        "adopt must reuse the existing store, not mint a duplicate"
    );
    ...
    // Sanity: the character is now linked to the (single) adopted store, and the
    // properties.json RMW applied the touched keys while preserving the others.
    let char_row = &got[0]["rows"].as_array().unwrap()[0];
    assert!(
        char_row["characterDocumentMountPointId"].is_string(),
        "character FK linked after adopt"
    );
    let docs_i = TABLES
        .iter()
        ...
        d["content"]
            ...
            .map(|c| {
                c.contains("\"title\": \"the spy\"")
                    && c.contains("\"firstMessage\": \"You didn't see me.\"")
    ...
    assert!(
        props_rmw,
        "properties.json RMW onto the adopted vault did not preserve untouched keys"
    );

    eprintln!("OK: characters adopt tier-2 matched oracle (6 tables, 2 DBs, single mount point).");
}
