//! Tier-2 differential test: the `doc_mount_files` repo — the **narrowest
//! mount-index sibling-DB repo** of Phase 2 (a required sha256 TEXT, one REAL-int
//! column, two enum TEXT columns; no JSON, no boolean, no nullable columns).
//!
...
//! `doc_mount_files` table canonically, and assert the post-op state is
...
//!     $N/npx tsx ~/source/quilltap-v5/harness/oracle/fixtures/build-doc-mount-files-fixture.ts
...
//! Run:
//!   QT_ORACLE_DOC_MOUNT_FILES=/tmp/oracle-dmf.ndjson \
//!   QT_FIXTURE_DOC_MOUNT_FILES=/tmp/qt-dmf-fixture.db \
//!     cargo test -p quilltap-harness --test doc_mount_files_tier2_equivalence

use std::path::{Path, PathBuf};

use quilltap_core::db::doc_mount_files::{CreateOptions, DmfCreate, DmfUpdate};
...
fn spec_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../harness/oracle/fixtures/doc-mount-files-tier2.json")
}

#[test]
fn doc_mount_files_tier2_matches_oracle() {
    let oracle_path = match std::env::var("QT_ORACLE_DOC_MOUNT_FILES") {
        Ok(p) => p,
        Err(_) => {
            eprintln!(
                "SKIP: set QT_ORACLE_DOC_MOUNT_FILES to the oracle NDJSON (see test header)."
            );
            return;
        }
    };
    let fixture = match std::env::var("QT_FIXTURE_DOC_MOUNT_FILES") {
        Ok(p) => p,
        Err(_) => {
            eprintln!(
                "SKIP: set QT_FIXTURE_DOC_MOUNT_FILES to the seed fixture .db (see test header)."
            );
            return;
        }
    };
    ...
    {
        let repo = writer.doc_mount_files();
        for op in &spec.ops {
            match op {
                ...
                Op::Update { id, data } => {
                    let found = repo
                        ...
                        .expect("doc_mount_files.update");
                    assert!(found, "update target {id} not found in fixture");
                }
                Op::Delete { id } => {
                    let removed = repo.delete(id).expect("doc_mount_files.delete");
    ...
    let got = writer
        .dump_table_json("doc_mount_files", "id")
        .expect("dump doc_mount_files");
    ...
    eprintln!("OK: doc_mount_files tier-2 matched oracle ({n} rows).");
}
