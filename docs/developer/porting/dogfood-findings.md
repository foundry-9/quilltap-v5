...
| 61 | The import wizard's step 2 renders an EMPTY body, and import-execute with `overwrite` then fails with a bare "Failed to import data" | **OPEN — not yet diagnosed; server-side line still owed.** Step 2 is the **preview** step (`import-dialog.ts:299-310` maps `preview` → 2), so an empty body means the preview response carried nothing the wizard could draw, and the execute failure is most likely downstream of the same empty preview rather than a second fault. Note both screenshots were taken on the post-restore scratch copy, whose state finding #56 had already damaged, so this must be re-walked on a FRESH copy before being taken at face value | **OPEN** — re-walk on a fresh copy first; if it reproduces, it belongs to the restore/import order below |
...
- **NEEDS AN ORDER (findings #57–#61, 2026-08-03) — the restore/import remainder,
  ...
  4. **Re-walk import** (#61) on a FRESH copy — the reports came from a scratch
     copy already damaged by #56, so the empty preview may be a symptom.
