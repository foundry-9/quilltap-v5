...
| 95 | On a local **Ollama** profile whose chat template enforces "System message must be at the beginning" (Qwen3.5-9B), every **non-initial** turn dies with `HTTP 500 … Jinja Exception: System message must be at the beginning`. The opening greeting succeeds (it carries ONE system message); the next turn carries **three** (`roles: [system, system, system, user, …]` — system prompt, identity reminder, compressed history) and the template refuses the whole request. On screen the turn simply never arrives | **Faithfully ported v4 behavior — NOT fixed.** v4 pushes the same three leading system blocks (`lib/chat/context-manager.ts:1902/1915/1926`), so v4 fails identically against the same model. Merging the leading system blocks for Ollama would be a v5-first divergence | **FILED UPSTREAM as v4 bug 82** (`docs/developer/bugs/bug-82-three-leading-system-messages.md`, v4 commit `93bd3e7c`). v5 stays faithful until v4 moves; this row retires when the port absorbs the fix. Found 2026-08-19 |
