...
|---|---|---|---|
| 99 | **The tool-execution notice never appears on a real `generate_image` call.** Bug 77's whole subject — the `Generating image…` banner above the composer, and the success/error notice that replaces it — does not render. Measured on three separate turns with a correctly-scoped 120–150 ms poller watching BOTH the composer's `[role=status]` and `qt-toast-container`: nothing, at any point in the turn | **A v5 client defect — the server side is provably correct, so this is below the SSE boundary.** The raw `/api/events` stream was recorded in the same turns and carries exactly what v4's hooks consume: `{chatId, toolsDetected: 1, toolNames: ["generate_image"], toolArguments: […]}`, then `{chatId, status: {stage: "tool_executing"}}`, then `{chatId, toolResult: {name: "generate_image", success: false}}`. Every frame carries the matching `chatId`, so the salon's `filter(frame => frame.chatId === chatId)` (`salon-conversation.ts:2732`) passes them. v4 raises the notice from two callbacks — `trackToolsDetected` publishes `Generating image...` the moment `toolNames` includes it (`useSSEStreaming.ts:380-387`, i.e. BEFORE execution, so the outcome is irrelevant), and `trackToolResult` publishes success or error plus a toast (`:414-435`). v5 ports both into `reportStreamTransitions` (`:2907`), which is wired on exactly the send path used (`:2737`, the `chatSend` composer path). So the frames arrive, the filter passes, the publisher is called on every frame — and nothing renders. ⚠ **The dropping layer is NOT isolated.** One probe worth following up: `document.querySelectorAll('qt-streaming-message')` stayed at **0 for the entire turn**, which would explain `toolBatches` rendering nothing, but that was not chased far enough to trust — the streaming overlay may simply mount under a different element, and the profile in use (`Grok 4 Fast Non-Reasoning`) may take a non-streaming path worth ruling out first | **OPEN — needs a focused session, not a dogfood patch.** Reproduction is cheap and free: any chat whose seats resolve **no** image profile, ask the model to call `generate_image`, and it refuses at the executor (`tools/executor.rs:1505`) without spending anything — the frames above still fly. Bisection order for whoever takes it: (0) **retest on a streaming profile first** — all three measured turns ran on `Grok 4 Fast Non-Reasoning`, and `qt-streaming-message` (which `message-list.ts:151` mounts on `@if (stream())`) was absent for the whole turn, so a non-streaming reply may be suppressing the live tool UI as well; a `DeepSeek V4 Flash Tools` seat is the control, (1) does `reduceChatFrame` actually push the batch (log `state.toolBatches` in the `:2733` subscribe), (2) does `reportStreamTransitions` reach the `generate_image` loop, (3) does `toolExecutionStatus()` get set but not render. **The blast radius is not yet bounded**: if `toolBatches` never populates client-side, the live in-transcript tool blocks are dark too, not just the banner — which is the difference between a patch and a lane. Note the existing coverage did not catch this: the bug-77 specs drive `reportStreamTransitions` directly with hand-built states (`salon-conversation.spec.ts:1963-1965`), so they prove the method and not the pipeline into it — the same "the production call could vanish unseen" gap the P4.D87 §3 review flagged for the turn-end teardown, reappearing on the entry side. Found 2026-08-21 |
...
- **A spec that drives a private method proves the method, not the pipeline into
  it.** Finding #99 is the second instance of one shape: the bug-77 coverage
  calls `reportStreamTransitions(before, after)` with hand-built states
  (`salon-conversation.spec.ts:1963-1965`), so it stays green while the frames
  that should reach it never arrive. The P4.D87 §3 review caught the same shape
  on the teardown side and pinned the wiring there; nobody pinned the entry
  side, and the entry side is what broke. **Before more SPA verticals land, the
  rule worth adopting is: any handler that reacts to a wire event needs at least
  one test that feeds it a real frame through the real reducer** — not a
  hand-built state — or an e2e beat that asserts the visible consequence. The
  cheap version is a reducer-level test (`reduceChatFrame` → assert the
  component's rendered output); the honest version is a beat. Either beats a
