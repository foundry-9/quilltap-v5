...
| 60 | Scheduled jobs kept running throughout the restore | **OPEN — a corruption risk, not a cosmetic one.** A job pump claiming and WRITING while a restore truncates and repopulates 43 tables across three partitions is writing into a moving target. The seam to do it with already exists and is already wired: `JobPumpControl` (the host seam P4.19 added, which the Tasks Queue Stop button drives and which genuinely halts claiming). v4's restore does not pause its runner either, but under the 2026-08-03 ruling above that is not a defence in this family | **OPEN — needs an order** (bundle with the restore work below). A progress indicator was also asked for, and is a fair nice-to-have on an operation this long |
