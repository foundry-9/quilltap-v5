...
| 59 | Restore reported 19 × `File not found in backup: <name>` | **OPEN — points at the BACKUP side, not the restore side.** These are files the archive's manifest references but whose bytes were not collected, so the complaint surfaces at restore. ⚠ Diagnose carefully: the files phase is already the subject of a ruled deliberate divergence (P4.9G5 — v4 cannot restore any user file into a fresh or wiped target in either mode, so v5 deliberately runs files AFTER the doc-store family), and `restore-archive-uploads.zip` / `restore-archive-gen2.zip` already exercise carried store rows. Establish whether these names are legacy disk-key files, store-backed blobs, or neither, BEFORE changing a phase | **OPEN — needs an order** |
| 58 | Restore reported ~50 × `Failed to restore doc-store file link "<name>": FOREIGN KEY constraint failed` (three names, repeated) | **OPEN — restore ordering or a missing parent row.** A `doc_mount_file_links` insert requires its `doc_mount_files` parent; the repetition across only three filenames (`physical-description.md`, `physical-prompts.json`, `images/avatar.webp`) suggests one character vault's files failed to land and every link into them then failed. Related to #59 but not obviously the same fault | **OPEN — needs an order** |
