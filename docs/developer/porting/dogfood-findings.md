...
| 6 | The Default Settings tab "doesn't seem to accept edits" on Friday data | TWO layers. Confirmed: a port divergence in error surfacing — v4 wraps every defaults save in try/catch + `showErrorToast` (`'Failed to update agent mode'`, …); v5's `save()` had `try/finally` with NO catch, so a server-rejected save silently snapped the control back with nothing shown. Underlying Friday failure: NOT yet reproduced — the tab saves + persists correctly against the fixture instance (verified live: agent-mode flip survived a full reload), so the actual rejection is real-data-specific and was invisible precisely because of the swallowed error | **PARTIAL** — error surfacing fixed (tab-level `qt-alert-error` with v4's fallback microcopy per control; unit tests for surfaced message + fallback). Awaiting the human's retry on the new build to read the real error; the server log line at the moment of the change is the other diagnostic |
