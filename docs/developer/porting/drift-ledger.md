...
- **Oracle baseline:** `b121ac77f` — "feat(cli): rebuild a lost or
  ...
  (2026-08-27).
- **Checked:** 2026-09-01 (a probe-failure update at the round-1 `/unify`;
  the previous full `/driftcheck` was 2026-08-31, with full checks
  2026-08-30, 2026-08-29 and 2026-08-27).
- **v4 `main` HEAD at check:** `4622411fd` — **SIXTEEN commits past the
  baseline**. One is new since the 2026-08-31 check: `4622411fd`
  (docs(release), one file `docs/releases/4.9.0.md`, zero
  lib/app/packages/plugins hunks — a NO-PORT? row, classified at the
  round-1 unify).
...
- **Verdict: DRIFT PENDING — 15 commits.** The backlog needs more than one
  catch-up round. Seven substantial ports: `1560bd43b` (Lima/WSL2
  retirement, six ported surfaces), `65f5021c8` (provider/model fallback
  chains — PORT-NEW, two `connection_profiles` columns), `a1d88aa3a`
  (bugs 106/107 — the reroute's message-array re-decision + the cheap-LLM
  budget rewrite), `487ae16b1` (bugs 108/109 — doc-edit argument guards +
  typographic folding, **v4's own bug row says 109 APPLIES to this
  port**), `84f33ce94` (LoRA adapters + per-model image options —
  PORT-NEW, with a five-call-site image-params consolidation that fixes
  drift v5 likely inherited), **`735d9408c` (bug 112 — `lastMessageAt`
  ...
  call sites, new Concierge notice sentences)**. Three smaller ports:
  `5f56f7a7d` (the `qt-range` class), `648d5c8aa` (bugs 110/111) and
  `2ece98c90` (the HuggingFace LoRA lookup — PORT-NEW), the last two
  stacked on the LoRA commit: **the LoRA train is now THREE commits,
  D-stacked** (`84f33ce94` → `648d5c8aa` → `2ece98c90`). Four docs-only
  NO-PORT? candidates (`3c3432ae9`, `97ebfb9fc`, `e41fcb12e`,
  `7819afb1d` — the third banks help hunks to `p4.9i2`) plus `7fb668263`
  (a Discord link that reaches the ported About mirror).
...
- **Regen rule in force: PIN REQUIRED** — v4 HEAD is fifteen commits past
...
| `735d9408c` | 2026-08-30 | feat(chats): date a chat by when a character last spoke (bug 112) | **PORT** | **Bug 112 — v4-filed (a human report, not this port's filing — NOT a convergence); its v5-status column reads "Not investigated. The shape applies."** `lastMessageAt` was bumped on any `type: 'message'` row, but Staff announcements, announcement bubbles and raw TOOL rows are ALSO message rows (that's what gives them avatars), so a story background finishing, a folded summary, a Pascal roll or a Host notice each floated a dead chat to the top — 608 of 871 chats mis-dated on the real instance. **(1) The new chokepoint** `lib/chat/chat-activity.ts` (+90): `isCharacterAuthoredMessage` (role USER/ASSISTANT, no `systemSender`, no `customAnnouncer` — whispers deliberately COUNT), `CHARACTER_AUTHORED_MESSAGE_FILTER` (the SQL mirror so the indexed lookup and the in-memory predicate cannot drift), `chatActivityAt` (`lastMessageAt ?? createdAt`, **never `updatedAt`**), `byChatActivityDesc` → a new v5 module + `db/chats_messages.rs` (both write sites; deletion now RECOMPUTES so the date walks backwards when the newest character message goes). **(2) Readers routed through it** — each a ported surface: `chat-enrichment.service.ts` → `services/chat_enrichment.rs` (**the P4.65 `ChatListPreloaded` batching + Salon list sort**), `home-data.service.ts` → `services/home.rs` (P4.64, family `home_routes_equivalence`), `app/api/v1/characters/[id]/handlers/get.ts` (its hand-rolled copy of the same mistake DELETED) → `api/characters.rs`, `app/api/v1/projects/[id]/actions/chats.ts` → `api/projects.rs`, `app/api/v1/brahma-console/route.ts` → `api/brahma.rs`, `getLastPlayedMessageAt` (staleness and display now agree by construction), `chat-utils.ts`, `self-inventory/builders.ts` → `tools/self_inventory.rs` (prompt-visible). **(3) Restore + import:** `restore.ts` re-derives `lastMessageAt` from the replayed transcript post-write (addMessage stamps wall clock — the whole history would land in one flat heap) with a named warning arm → v5 restore + `system_restore` families; `ai-import.service.ts` filters through the predicate instead of trusting the last row. **(4) The data migration** `recompute-chat-last-message-at-v1` rewrites the column for existing chats — v5 has no migration runner; the P4.D97 retire-prefill heal (over v4's own `migrations_state` ledger) is the precedent shape for the ordering lane. **DDL.md is comment-only — NO schema change, no D23 re-dump.** **(5) SPA:** `ChatCard`/`MergeConversationModal`/`RecentChatItem` + Prospero `ChatsSection` display `chatActivityAt` → v5's chat cards, merge modal, home recent items, project chats. ⚠ e2e/fixture builders that stamp wall-clock `lastMessageAt` (the standing P4.D131 memory note) may need the same predicate. `help/chats.md` → bank `p4.9i2`. NO-PORT remainder: README, CHANGELOG, bugs docs, v4's own four test files, `package*.json`. | UNPROCESSED |
| `e41fcb12e` | 2026-08-30 | docs(help): why a correctly-configured LoRA can still paint a tame picture | **NO-PORT?** | CHANGELOG + `help/{dangerous-content,image-generation-profiles}.md` only — zero `lib/`/`app/`/`packages/` hunks. The help hunks bank to `p4.9i2` with the rest of the LoRA train's help rows. Ratify with evidence at the catch-up round. | UNPROCESSED |
| `2ece98c90` | 2026-08-30 | feat(images): query a LoRA source against HuggingFace from the profile editor | **PORT-NEW** (stacked on `84f33ce94` — the LoRA train's third commit) | **The repo's second mocked non-LLM external HTTP provider after Serper (P4.42's `mock-serper.ts` is the precedent).** NEW `lib/image-gen/huggingface-lookup.ts` (+263): asks `https://huggingface.co/api/models` what it knows about a LoRA source — resolves/gated, weights files, `base_model`, and `cardData.instance_prompt` (the trigger phrase, the reason the feature earns a button) — **deliberately renders NO compatibility verdict** (the module doc says a false "this will not work" is worse than silence); 10 s timeout; host-side only so egress stays in one place and a gated-weights token never reaches the page. NEW `huggingface-repo-id.ts` (+56, `extractHuggingFaceRepoId`/`huggingFaceCardUrl`). **Wire:** `POST /api/v1/image-profiles?action=lora-metadata` (POST not GET **because `hfToken` is a credential and doesn't belong in a query string** — carry that comment); hand-rolled body guards (`'A JSON body with a `source` is required'`, non-object/array refused) → v5 `api/image_profiles.rs` + family `image_profiles_routes_equivalence` (P4.9H2B / `84f33ce94`'s row). **SPA:** NEW `LoraQueryResult.tsx` (+187), `LoraListEditor.tsx` (+120 — the query button + trigger-phrase adoption), `ImageProfileForm.tsx` → the v5 LoRA editor from the `84f33ce94` order. `help/image-generation-profiles.md` → bank `p4.9i2`. NO-PORT remainder: README, CHANGELOG, API/feature docs, v4's own 327-line test file (its fetch-mock shapes are the differential's corpus shape), `package*.json`. | UNPROCESSED |
| `4622411fd` | 2026-08-31 | docs(release): Update to 4.9.0 release notes | **NO-PORT?** | `docs/releases/4.9.0.md` only (+70/−6), one file, zero `lib/`/`app/`/`packages/`/`plugins/` hunks — the same class as `3c3432ae9`. Arrived mid-round (during the round-1 catch-up lanes); classified at that round's `/unify`. The 4.9.0 release-notes file keeps moving — the squash may be imminent. Ratify with evidence at a catch-up round. | UNPROCESSED |
| `60e3c4a0a` | 2026-08-31 | feat(concierge): four-state per-chat control (route x provenance) | **PORT** | **The per-chat Concierge control grows from three states to four** — NULL (classifier decides Monitored/Flagged), `'OFF'` relabeled **Vouched Safe** (no Concierge effects, ordinary providers), NEW `'UNCENSORED'` (operator-asserted: every uncensored route the Flagged state takes, zero classification, zero danger styling); both operator states preserve `isDangerousChat` underneath so resuming picks up where the classifier left off. **(1) Schema — NO DDL change, no D23 re-dump:** the migration `widen-concierge-override-domain-v1` is **ledger-only** (TEXT column, no CHECK; zero rows touched — it exists so downgrade detection and the documented schema stay honest); the Zod enum widens in `lib/schemas/chat.types.ts` + `app/api/v1/chats/[id]/schemas.ts` + `public/schemas/qtap-export.schema.json` → v5's chat verbs + export carry. **(2) The dangerous-content family reshapes** — all three modules v5 ported leaf-to-root: `chat-override.ts` (+113/−…, `isChatActiveDangerous` → **`shouldUseUncensoredRoute`** renamed AND re-semanticized), `manual-flip.ts` (the flip kinds change: `manual-off-duty`/`manual-on-duty` → `manual-vouched`/`manual-resumed`/`manual-uncensored`), `resolver.service.ts` ("Safe" renamed **Monitored** throughout) → `quilltap-core/src/services/dangerous_content/{chat_override,manual_flip,resolver}.rs`, families `danger_resolver_equivalence` (carries the manual-flip cases), `danger_scan_tier2_equivalence`, `danger_gatekeeper_tier3_equivalence`, `danger_routing_equivalence`. **(3) ~14 call sites thread the new predicate/type** — eight background-job handlers (`carina-memory-extraction`, `chat-danger-classification`, `memory-extraction`, `memory-regenerate-all`, `scene-state-tracking`, `story-background`, `title-update`, `scheduled-danger-scan`), `context-summary.ts`, `pascal/llm-consult.ts`, `chat-enrichment`, the spine quartet (`danger-orchestrator`, `message-finalizer`, `orchestrator`, `pre-compute`), `image-generation-handler.ts` → v5's twins (`orchestrator.rs`, `context_summary.rs`, `danger_scan.rs`, `enclave/{step,lifecycle}.rs`, `api/salon.rs`, the job handlers). **(4) New Concierge notice sentences** (Wodehouse register, byte-exact port targets): the vouched sentence reworded + a NEW uncensored-door sentence shared by `manual-resumed`/`manual-uncensored`, and the operator-advisory strings split → `concierge-notifications/writer.ts` twin, family `post_office_concierge_lantern_suparna_equivalence`. **(5) SPA:** `SalonView` + `ChatSidebar` grow the four-state control (+56), `_chat.css` +22 (v5's qt-components css needs the same classes) → the Salon vertical + sidebar; `packages/theme-storybook/**` NO-PORT (v5 ships no storybook). **(6)** v4's e2e script renamed `…tristate-test.sh` → `concierge-four-state-test.sh` — the shape for v5's beat. `help/dangerous-content.md` → bank `p4.9i2`. ⚠ the four-state feature doc (`docs/developer/features/complete/concierge-four-state.md`, +403) is the design record — read it at ordering, but port from hunks (§5.3). NO-PORT remainder: README, CHANGELOG, DDL comment, v4's own tests, `package*.json`. | UNPROCESSED |
