...
- **Checkout at the `/setupphase` planning close (2026-09-16, later that
  evening):** branch **`main`**, HEAD still `53294163f`, tree **DIRTY** — a
  v4 bug-150 fix in flight, uncommitted: `components/images/image-
  generation-dialog.tsx` (M), `__tests__/unit/image-generation-dialog.test.
  ts` → `.tsx` (RM), `docs/developer/bugs/bug-150-generation-dialog-dead-
  endpoint.md` → `docs/developer/bugs/fixed/` (RM), `docs/developer/bugs.md`
  (M), `docs/CHANGELOG.md` (M). Client-only dirt on a file v5 has no twin
  for (`grep -rn 'image-generation-dialog\|ImageGenerationDialog' apps/web/
  src` is empty — P4.D197 measures it); the regen rule was already PIN
  REQUIRED, and this dirt is the second reason it stays so for every lane.
  **A §2 probe that sees exactly these five paths dirty and an unmoved HEAD
  PASSES; anything more is a STOP.** When the fix is committed, the next
  `/driftcheck` classifies it (expected NO-COUNTERPART for v5).
