...
- **Oracle baseline:** `7fb668263` — "doc(discord): Updated Discord link"
  (v4 main, 2026-08-29), adopted at the round-1 drift catch-up
  unification (P4.D134 ∥ P4.D135→P4.D136 ∥ P4.D137, 2026-09-01). The
  round absorbed the contiguous eight-row prefix `1560bd43b..7fb668263`.
- **Checked:** 2026-09-01 (at the round-1 `/unify` — the §2 probe plus the
  baseline move; the previous full `/driftcheck` was 2026-08-31).
- **v4 `main` HEAD at check:** `4622411fd` — **EIGHT commits past the new
  baseline** (the round-2 backlog). One arrived mid-round: `4622411fd`
...
- **Verdict: DRIFT PENDING — 8 commits (round 2 of the pre-planned pair).**
...
- **Regen rule in force: PIN REQUIRED** — v4 HEAD is eight commits past
  the baseline, so every oracle regen runs from a worktree pinned at
  `7fb668263` (§5.1) until the round-2 catch-up moves the baseline.
