...
- **Checked:** 2026-09-21, 22:25 — **the THIRD standalone `/driftcheck` that
  day**, run because v4 landed three more commits between 19:37 and 22:20,
  the last of them **five minutes before this check**. Previously checked
  2026-09-21 18:xx (the FTS5 escalation) and 17:xx (the nine-commit check),
  2026-09-19 twice, 2026-09-18 (the round's `/unify`).
- **v4 `main` HEAD at check:** `a2db63da7` ("Fix bug 162: CLI raw-SQL path
  opens without qt_text() (#63)", 2026-09-21 22:20, `4.10.0-dev.63`) —
  **SIXTEEN commits past the baseline.** Thirteen of the sixteen landed on
  2026-09-21 between 10:14 and 17:25 and are already **ORDERED** into the
  catch-up round now in flight; the **three newest are NEW at this check**
  and are UNPROCESSED.
...
- **Verdict: DRIFT PENDING — 16 commits: 13 ORDERED, 3 NEW.** The three new
  rows are v4's own bugs 161 and 162 and the docs commit that filed them,
  all three out of ONE live `Friday` chat whose running summary had been
  calling a character **"Vivienne"** — a name in no message and on no
  character:
  - `4e1a8e061` (19:37, docs-only) files both bugs and writes the design of
    record. **NO-PORT?**, with a mirror: the spec is the best reference the
    161 port has.
  - `e7821606f` (22:07, bug 161) is a **PORT on a surface v5 reproduces
    verbatim** — measured, not inferred: v5's fold renders
    `m.role.to_uppercase()` (`services/context_summary/tasks.rs:82`) under a
    prompt whose last line is still v4's pre-fix *"Use character names, not
    roles."* (`services/context_summary/prompt_text.rs:20`), and v5 carries
    the SAME duplicated seat-name loop v4 has now deleted
    (`services/fold_episode_pass.rs:150-190`). It also carries a
    **PORT-NEW** half: `?action=rebuild-summary` + its Salon menu entry.
  - `a2db63da7` (22:20, bug 162) is a **PORT** on the CLI's `db` opener, and
    it **ENLARGES the codec keystone's brief** (see two bullets down).
- **⚠ THE CATCH-UP ROUND IS IN FLIGHT.** Eight lane worktrees are live
  (`p4-d203` text codec, `p4-d205` Inform server, `p4-d206` Salon SPA,
  `p4-d207` swipe stream, `p4-d208` scenario-seeded, `p4-d211` zod 4.6,
  plus the FTS5 and Scriptorium lanes) against the thirteen ORDERED rows.
  **The three new rows are OUTSIDE that round's scope and must not be added
  to a running lane.** Two of them land near lanes that are running, so the
  unifier should expect to sequence them rather than merge them:
  `e7821606f` touches the Salon sidebar's Organize drawer (P4.D206's v5
  surface, `apps/web/src/app/chat/sidebar/organize-section.ts`) and
  `a2db63da7` touches the CLI opener that P4.D203's UDF must reach.
- **⛔⛔ BLOCKING, ESCALATED, PROVEN BY EXPERIMENT — AND NOW CORROBORATED
  FROM V4'S OWN SIDE: v5 CANNOT WRITE A CHAT MESSAGE to a v4-4.10
  instance.** `f45a517a9` puts three triggers over `chat_messages`, and
  **two of them call `qt_text()`** — `_ai`'s INSERT body and `_au`'s
  **`WHEN` clause** (`qt_text(new."content") IS NOT qt_text(old."content")`).
  v5 registers no UDF anywhere (zero hits for
  ...
  **⭐ v4 hit the identical wall four days later from its own CLI and filed
  it as bug 162** (`a2db63da7`): a connection opened without the UDF "could
  neither read a compressed column nor write a `chat_messages` row". That is
  independent confirmation of the measurement below, and it supplies the
  shape of the remedy v4 chose — **one opener for every connection, with the
  registration inside it** (`registerTextCodecFunction` inside
  `openEncryptedDb`, `packages/quilltap/lib/db-helpers.js:179-217`), taken
  deliberately over a second registration call "because two openers is how
  this happened". P4.D203 should read that as the sanctioned placement.
...
- **Regen rule: PIN REQUIRED at `baa85e19b`**, absolutely and unchanged by
  this check. Sixteen commits of divergence, five of them rewriting paths
  whose families regenerate routinely (search, the swipe, the mount-index
  write path, chat creation, and now **the context-summary fold and the
  episode pass** — `QT_ORACLE_FOLD_EPISODE` and the context-summary
  families move at `e7821606f`). Every oracle regen until the baseline moves
  runs from a detached worktree pinned at `baa85e19b` (§5.1, lane-unique
  path). **Do not reason about which families "could" be affected — pin.**
- **⚠ The workspace gate MOVES in TWO places, both by design, both measured
  ...
  `6b0615807` (ORDERED to P4.D211); its own failure text names the P4.D158
  re-measurement and the regen it gates. **Green:**
  `public_schemas_vendor_guard`, and `help_tree_embed_guard` — the latter
  only because it compares embedded against v5's own DISK rather than
  against v4, so it cannot see the gap below. `help_tree_equivalence` is the
  only family that can.
- **Schema state: a D23 re-dump is OWED from `e7d77bb60` and from that
  ...
  risk**. The cost is entirely in the UDF and the codec. **Neither of the
  three new commits moves the schema** — `e7821606f` ships no migration on
  purpose (v4's own reasoning: a correct name and an invented one are not
  mechanically distinguishable, so the remedy is an operator action, not a
  sweep), and `a2db63da7` is bin-only.
- **`help/**` is BEHIND v4 in ELEVEN files, re-measured 2026-09-21 after
  `a2db63da7`:** v5 has **124**, v4 has **126**. **MISSING (2):**
  `inform.md` (`e7d77bb60`), `cli-sync.md` (`23da0b322`). **DIFFERS (9):**
  ...
  (`186eb09cb`), `search.md` (`f45a517a9`), **`chats.md` (`e7821606f`, +24 —
  new at this check)**. Every other shared file is still `cmp`-identical and
  v5 has nothing extra. `help_tree_embed_guard.rs:56` pins
  `VENDORED_FILE_COUNT = 124`, which moves with the re-vendor.
