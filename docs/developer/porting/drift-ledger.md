...
- **Checked:** 2026-09-19, 17:xx (a second standalone `/driftcheck` the same
  evening — v4 added a third commit after the first check). Previously
  checked 2026-09-19 (the first standalone check, which recorded the two
  Inform commits), 2026-09-18 (the `baa85e19b` round's `/unify`, opening and
  closing probes both PASSED), 2026-09-18 (the standalone post-`89fcc3c0d`
  `/driftcheck`).
- **v4 `main` HEAD at check:** `f564b0de3` ("feat(salon): a regeneration says
  so while it happens", 2026-09-19 17:00, `4.10.0-dev.51`) — **THREE commits
  past the baseline** (`git log baa85e19b..main` = `781e3b499`, `e7d77bb60`,
  `f564b0de3`).
...
- **Verdict: DRIFT PENDING — 3 commits.** Two are a **large PORT-NEW**
  feature and its plan doc (Inform: 79 files, +4,758/−38 — a new table, a new
  prompt-path system block, three API actions, two new SPA components, and
  export / import / backup / realtime / migration reach); the third is a
  **PORT** on the regenerate/swipe path (26 files, +1,226/−81 — the swipe's
  one provider call becomes a watched STREAM with a new SSE mode, and the
  Salon narrates it). All three rows are UNPROCESSED in §3. **v4 shipped all
  three inside three hours on one afternoon — expect more before a round
  closes, and re-probe rather than trusting this paragraph's age.**
- **Regen rule: PIN REQUIRED at `baa85e19b`.** v4's HEAD is three commits
  past the baseline, so a regen run from the checkout itself would import
  both features. Every oracle regen until the baseline moves runs from a
  detached worktree pinned at `baa85e19b` (§5.1's recipe, lane-unique path).
  ⚠ Inform is *empty-is-absent* on the prompt path (v4's own commit message:
  with nothing pending the builder pushes nothing, and neither
  builder-version constant moves), so an **unpinned** regen of a
  context/orchestrator family would very likely still come back
  byte-identical — the pin's protection there is against the parts that are
  NOT conditional (`extractVisibleConversation` now skips by KIND as well as
  role; `buildMessageContext`'s Commonplace strip moved behind a named
  `isRecordOnlyMessage` predicate; the Courier transport gained a strip).
  `f564b0de3` is the opposite case and needs no such caveat: it rewrites the
  swipe's generation call outright, so **any** regenerate/swipe family
  regenerated unpinned would come back wrong. Do not reason about which
  families "could" be affected — pin, per §5.1.
- **⚠ The workspace gate MOVES: `qtap_schema_embed_guard` goes RED against
  ...
  **10,890 → 10,989** (Inform's export rows) at `e7d77bb60`; `f564b0de3` did
  **not** move `public/schemas/` further. v5's vendored copy
  ...
  stays green.
- **Schema state: DRIFTED — a D23 re-dump is OWED** (from `e7d77bb60`;
  `f564b0de3` touches no DDL source). `e7d77bb60` adds the **`chat_informs`**
  table to the **main** partition via a standard `AbstractBaseRepository`
  over a Zod schema (`lib/schemas/chat-inform.types.ts`,
...
- **`help/**` is BEHIND v4 in THREE files, re-measured 2026-09-19 after
  `f564b0de3`:** v5 has 124 files, v4 has 125. `help/inform.md` is **missing**
  here (new at `e7d77bb60`); `help/insert-announcement.md` **DIFFERS** (+4 at
  `e7d77bb60`); `help/chat-message-actions.md` **DIFFERS** (+36 at
  `f564b0de3`). Every other shared file is still `cmp`-identical. Two
  re-vendor obligations, one per feature round.
