...
- **Checked:** 2026-09-24, late afternoon (`/driftcheck`, main-checkout
  ...
  the human's local branches were not touched). Second check of the day; the
  first (same afternoon) recorded HEAD `ad1c4c37f`.
- **v4 `main` HEAD at check:** **`f51a44548`** (`4.10.0-dev.77`) — local
  `main` = `origin/main`. **NINE non-merge commits past the baseline** (§3)
...
- **Checkout at check:** branch **`main`**, tree **CLEAN**, local `main` AT
  `origin/main` (`f51a44548`). §2's probe runs against `f51a44548`.
- **Verdict: DRIFT PENDING — 9 commits** (+ one clean merge) (§3): PORT ×4
...
- **Schema state:** unchanged — the D23 re-dump from `e7d77bb60` stands; the
  FTS objects come from P4.D204's boot reconciler. None of the nine commits
