...
- **Oracle baseline: `c2232cd9a`** — "dev: started 4.10.0 development" (v4
  ...
  2026-09-05) on P4.77's P4.D159 ratification evidence (zero ported-code
  bytes between `d883a5ee1` and `c2232cd9a`; the four release commits are
  NO-PORT-RATIFIED in §6).
- **Checked:** 2026-09-05, the unification's §2 probe (v4 on `bugfix`, tree
  clean, `git log c2232cd9a..main` empty, `git log 2b49f51aa..bugfix` empty).
- **v4 `main` HEAD at check:** `c2232cd9a` — **equal to the baseline.**
- **v4 `bugfix` tip at check:** `2b49f51aa` ("bugfix: started 4.9.0 bug
  branch", `4.9.1-bugfix.0`) — code-identical to `main` (measured at the
  last `/driftcheck`; re-measure by CONTENT at the next).
- **v4 `release` tip at check:** `b0eea4642` ("release: 4.9.0").
- **Checkout at check:** branch **`bugfix`** (not `main`), tree CLEAN.
- **Verdict: NO DRIFT** — v4 HEAD equals the baseline.
- **`generateDDL` and `lib/database/schema` UNTOUCHED** since the last D23
  re-dump; no re-dump owed.
- **Regen rule in force: PIN REQUIRED at `c2232cd9a` — for ONE reason only:
  the checkout sits on `bugfix`** (§5.1's second trigger). The code is
  byte-identical across `main`/`bugfix`/`release`; the one exposure is the
  version string (`4.9.1-bugfix.0` vs `4.10.0-dev.0`), and the three
  version-emitting families are version-transparent by construction (the
  previous §1's finding, still true). **The rule returns to PIN NOT REQUIRED
  the moment the human puts the checkout back on `main`** — v4 is the human's
  active repo: ask, never switch it.
- **Standing hazards that SURVIVE every baseline move (re-read before any
  ...
  `token-estimation` at any sha BEFORE `d4138b96b` would not match; (4)
  `packages/quilltap/package.json` is stale at `4.9.0-dev.135` on both v4
  branches (v4-side nit; no `--version` comparand exists, Tier R unaffected).
- **Release shape:** v4 develops on `main` at 4.10.0-dev with a live 4.9.1
  `bugfix` fork (`2b49f51aa`, code-identical to `main` at this check); §4
  step 2's two-branch rule is load-bearing — measure `bugfix` by CONTENT,
  never its commit list.
- _Superseded (the 2026-09-05 `/driftcheck` before this round): DRIFT PENDING
  — 4 docs/version-only commits past `d883a5ee1`, PIN REQUIRED; absorbed as
  NO-PORT-RATIFIED at this unification._
