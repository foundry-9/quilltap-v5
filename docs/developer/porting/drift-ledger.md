...
|---|---|---|---|---|---|
| `44a8137e9` | 2026-08-25 | feat(salon): the scene can be changed without leaving the conversation | PORT-NEW | New verb surface on ported ground: `app/api/v1/chats/[id]/actions/scenario.ts` + schemas (v5's chat dispatch + routes families), a large `NewChatForm.tsx`/`new-chat/types.ts` refactor extracting `ScenarioSelect` (the ported New-Chat vertical — P4.D44's template picker lives there), `ChatSidebar`/`SalonView` riders (ported Salon SPA), `ChatScenarioControl.tsx` new. `help/` changes → the `p4.9i2` bank. | UNPROCESSED |
...
| `6afacb187` | 2026-08-25 | fix(cli): shell completion survives flags on the line (bug 101) | PORT | `packages/quilltap/lib/completion/{bash,fish,zsh}.template` rewritten wholesale — v5 byte-copied these at P4.D68/P4.D97 (Tier R). New `completion-behavior.test.js` drives bash for real. | UNPROCESSED |
| `309aaa97a` | 2026-08-25 | fix(themes): hover and opacity qt-* classes actually style something (bugs 100, 102) | PORT | `_utilities.css` +490 lines (missing opacity steps, STATE VARIANTS section, `qt-text-on-*` family) + ~24 component call-site class rewrites across ported SPA surfaces + the new `scripts/check-qt-classes.mjs` lint guard. v5 ported the qt-* utility sheet at P4.D34 and likely inherits the inert names (bug 100/102's own notes say so). | UNPROCESSED |
| `8f9101370` | 2026-08-25 | fix(ci): the zsh completion check no longer fails where zsh isn't installed | NO-PORT? | `.github/workflows/ci.yml` + a tests-only tweak to `completion-behavior.test.js`. v5 has no zsh CI check. Ratify at absorption. | UNPROCESSED |
