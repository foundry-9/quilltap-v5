...
- **Checked:** 2026-09-21, 17:xx (a standalone `/driftcheck`). Previously
  checked 2026-09-19 twice (the two Inform checks), 2026-09-18 (the
  `baa85e19b` round's `/unify`), 2026-09-18 (the standalone post-`89fcc3c0d`
  check).
- **v4 `main` HEAD at check:** `80a05a4c8` ("docs: check the message FTS5 +
  compression plan against the code", 2026-09-21 16:47, `4.10.0-dev.59`) —
  **TWELVE commits past the baseline.** Nine of them are NEW since the last
  check and **all nine landed on 2026-09-21**, between 10:14 and 16:47:
  `0ecc6067c`, `23da0b322`, `0c14fd61f`, `5e4252898`, `e11a51f44`,
  `6b0615807`, `da9c4f34f`, `186eb09cb`, `80a05a4c8` — on top of the three
  already recorded (`781e3b499`, `e7d77bb60`, `f564b0de3`). **v4 is shipping
  faster than this port can absorb; re-probe, never trust this paragraph's
  age.**
...
- **Checkout at check:** branch **`main`**, tree **CLEAN at the moment of
  this check's `git status`** — but ⚠ **the human confirmed live work is in
  flight in the v4 repo on 2026-09-21, and a parallel read during this very
  check saw the tree DIRTY in `lib/` with FTS5 PR-1 files**
  (`lib/database/backends/sqlite/chat-message-fts.ts`,
  `lib/database/repositories/fts-query.ts`,
  `lib/startup/reconcile-chat-message-fts.ts`, plus a modified
  `chats-search.ops.ts`). **This check classifies COMMITTED history only, as
  instructed.** Treat a dirty probe as expected, not as a new finding — but
  it is one more reason the regen rule below is absolute. No fetch was run;
  the oracle imports the local checkout, so local HEAD is what the regen
  rule is about.
- **Verdict: DRIFT PENDING — 12 commits**, and the backlog is now both
  larger and *qualitatively* worse than any previous drift. Four rows are
  docs-only NO-PORT? candidates (`781e3b499`, `0ecc6067c`, `5e4252898`,
  `e11a51f44`, `80a05a4c8` — five, counting Inform's plan). The rest:
  **`186eb09cb` is the most dangerous single drift this port has faced** (a
  new on-disk text codec — see the blocking note below); `e7d77bb60`
  (Inform) is the largest (79 files, a new table); `23da0b322`
  (`quilltap sync`) is a whole new CLI verb plus a 2,127-line engine with no
  v5 analogue at all; `f564b0de3` rewrites the swipe's generation as a
  watched stream (and closes a v5 tracked deferral); `da9c4f34f` (bug 158)
  and `0c14fd61f` (bug 157) are defects **v5 measurably reproduces**; and
  `6b0615807` is a "chore" that moves the Zod version the gate pins. All
  twelve rows are UNPROCESSED in §3.
- **⛔ BLOCKING — `186eb09cb` changed the BYTES IN COLUMNS v5 READS, and the
  human's LIVE Friday instance is ALREADY MIGRATED. MEASURED 2026-09-21 on a
  read-only copy (since deleted, `.dbkey` with it):** `llm_logs.request` =
  **5,627 BLOB / 10 text**, every blob carrying the header `51 01 01`;
  `llm_logs.response` = **4,475 BLOB / 1,162 text**;
  `conversation_chunks.content` = **14,962 BLOB / 177 text**. v4 now stores
  those three columns brotli-compressed behind a 3-byte header
  (`magic 0x51 / version 0x01 / codec 0x01`, brotli quality 5, values under
  **512 bytes left as plaintext**, so a column holds a mix forever by
  design) and decodes raw SQL through a new `qt_text()` UDF. **v5 binds
  `String` / `Option<String>` at every read site of all three** — e.g.
  `crates/quilltap-core/src/db/llm_logs.rs:1187-1188` — which rejects a BLOB
  with `InvalidColumnType`; **and at the two `llm_logs` sites the error is
  deliberately swallowed as v4's Zod-drop** (`:648` `.ok()`, `:780`
  `if let Ok`), so the LLM Inspector returns an **empty list and 404s every
  log with nothing in the UI or the log to say why**, and
  `services/backup/collect.rs:627`'s `.unwrap_or_default()` means **a
  v5-taken backup of a migrated instance silently omits every llm_logs
  row.** v5 also has **no UDF machinery at all**: the workspace pins
  `rusqlite` with `default-features = false, features = ["buildtime_bindgen"]`
  (`Cargo.toml:54`) and **no crate enables `functions`**, so `qt_text` is not
  merely unported, it is not currently compilable. **There is no migration to
  gate on** — new writes already compress, so a single v4 turn on a shared
  instance is enough. **The dogfood Friday copy is now untrustworthy for
  llm_logs and conversation-chunk reads, and v5-taken backups of it are
  incomplete.** ⚠ And the escalation is already in v4's working tree: the
  in-flight FTS5 work says in its own module doc that it unblocks
  compressing **`chat_messages.content`** — the column every chat read
  touches.
- **Regen rule: PIN REQUIRED at `baa85e19b`**, absolutely and with no
  case-by-case reasoning. v4's HEAD is twelve commits past the baseline AND
  the checkout carries uncommitted FTS5 work, so an unpinned regen imports
  both shipped features and unshipped ones. Every oracle regen until the
  baseline moves runs from a detached worktree pinned at `baa85e19b` (§5.1's
  recipe, lane-unique path). The Inform caveat recorded at the previous
  check still stands (it is *empty-is-absent* on the prompt path, so an
  unpinned context/orchestrator regen would often come back byte-identical
  and thereby hide the parts that are NOT conditional) — but `f564b0de3`,
  `23da0b322`, `da9c4f34f` and `186eb09cb` are all the opposite case. **Do
  not reason about which families "could" be affected — pin, per §5.1.**
- **⚠ The workspace gate MOVES in TWO places now, both by design, both
  measured red 2026-09-21 by running them:**
  **(a) `qtap_schema_embed_guard`** — `the_embedded_schema_equals_the_v4_
  checkouts` FAILS. `public/schemas/qtap-export.schema.json` grew
  **93,384 → 95,266** and `qtap-export-ndjson.schema.json`
  **10,890 → 10,989** at `e7d77bb60` (Inform's export rows); nothing since
  moved them further. That redness IS the Inform re-vendor obligation, not a
  gate fix.
  **(b) `zod_version_guard`** — `v4s_installed_zod_matches_the_recorded_
  version` FAILS, `4.5.4 → 4.6.5`, from `6b0615807`. This is exactly the
  event P4.77 built the tripwire for, and its own failure text names the
  obligation: repeat the **P4.D158 unit 2 item 4** three-part measurement
  (`zod/v4/locales/en.js` sentences, `zod/v4/core/regexes.js` regexes, a grep
  for any newly-reachable validator), then regenerate the two hand-rolled
  engines, the SPA corpus and the ~150 Zod-sentence/regex edge sites, *then*
  bump `RECORDED_ZOD_VERSION`.
  **Green, and worth recording as green:** `public_schemas_vendor_guard` (the
  SPA-served `qtap-custom-tool`/`qtap-progression` pair did not move) and
  `help_tree_embed_guard` — the latter **only because it compares embedded
  against v5's own DISK, never against v4**, so it cannot see the help gap
  below. `help_tree_equivalence` is the only family that can.
- **Schema state: DRIFTED — a D23 re-dump is OWED, from `e7d77bb60` and
  from that commit ALONE.** `e7d77bb60` adds the **`chat_informs`** table to
  the **main** partition via a standard `AbstractBaseRepository` over a Zod
  schema, so v4's live `generateDDL` now emits it and
  `crates/quilltap-core/src/services/provisioning/fresh_schema.json` must be
  re-dumped from v4 (D23 — never by hand). Ten columns (`id`, `chatId`,
  `batchId`, `participantId`, `contentMarkdown`, `recordMessageId`,
  `createdAt`, `updatedAt`, `consumedAt`, `consumedByMessageId`), a
  `chatId → chats(id) ON DELETE CASCADE` foreign key, three indexes; the
  migration is `add-chat-informs-table-v1`. **Measured this check: none of
  the nine new commits adds a table, a column or an index** — `23da0b322`'s
  `DDL.md +6` is three prose paragraphs, `0c14fd61f` touches `DDL.md` not at
  all, `da9c4f34f`'s `+1/−1` is a column comment, and `186eb09cb`'s `+69` is
  the new "Compressed text BLOB format" documentation (the columns stay
  `TEXT`; SQLite is dynamically typed, which is precisely why the codec
  needs no DDL change and precisely why it is dangerous). **So the re-dump is
  still one obligation, not five** — but `80a05a4c8`'s plan says the *next*
  FTS5 commit adds an id-mapping table, which will owe another.
- **`help/**` is BEHIND v4 in NINE files now, re-measured 2026-09-21:** v5
  has **124**, v4 has **126**. **MISSING:** `inform.md` (`e7d77bb60`),
  `cli-sync.md` (`23da0b322`). **DIFFERS (seven):**
  ...
  (`186eb09cb`). Every other shared file is still `cmp`-identical and v5 has
  nothing extra. `help_tree_embed_guard.rs:56` pins
  `VENDORED_FILE_COUNT = 124`, so that constant moves with the re-vendor.
- **✅ RETIRED: the standing `openai 7.10.0` item is RESOLVED by
  `6b0615807`** — and the standing note's count was wrong. Measured
  installed-vs-declared across every `plugins/dist/*`: **`openai@7.20.0`
  against `^7.20.0` in SIX dirs** (`deepseek`, `grok`, `nanogpt`,
  `openai-compatible`, `openai`, `z-ai` — six, not four), plus
  `@anthropic-ai/sdk@0.115.0`, `@google/genai@1.52.0`,
  `@openrouter/sdk@1.3.11`, all matching their declarations. The old bundles
  were at **two different versions** (7.10.0 and 7.15.0). The human
  `npm install` item recorded in phase-4.md can be struck at the baseline
  move, with the correction recorded.
