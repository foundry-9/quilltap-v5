...
| **W4.7c** — request builders + transforms + tool wire — **PART 1 (tool wire) DONE** (2026-07-07) | **DONE:** the tool wire (`crate::model::tool_wire`) — `formatTools` reshape per provider (closes W4.1g: `tool_build::format_tools_for_provider`, available + tested, wiring into `build_tools` a spine handoff), `parseToolCalls` per provider (closes the native-loop `ToolCallDetector`: `RegistryToolCallDetector`), and the provider text-markers strategy (closes the text-tool-loop seam: `ProviderTextMarkersStrategy`), all dispatched by the manifest `toolFormat`; the one backreference regex hand-rolled. **PART 2 (deferred):** the per-provider request-envelope builders; the four `RequestTransform` hooks (anthropic breakpoints + adaptive-thinking/sampling-rejection for Sonnet 5/Opus 4.7+/Fable/Mythos — port from **current** v4, consider lifting the model list to manifest data; openai `previous_response_id` chaining + fallback; google schema sanitizer + `thoughtSignature` round-trip; deepseek `reasoning_content` echo). | **PART 1 verified:** `tool_wire_equivalence` (231 rows tier-1 byte-exact vs v4's real plugin methods over the real b.3 catalog + recorded rawResponses, via `record-tool-wire.mjs`) + `native_tool_loop_tier3` / `text_tool_loop_tier3` regenerated with the real detector/strategy. **PART 2:** byte-diff of built request JSON vs v4 (extend the fetch-mock to capture the request); then wire the real detector/reshape into the `orchestrator` spine (regenerate its oracle with the real registry). |
