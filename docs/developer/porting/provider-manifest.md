...
|---|---|---|---|
| **builtin** (`tfidf-bm25-v1`) | none — **local, in-process** | n/a | `LocalEmbeddingProvider`: TF-IDF + BM25 + Porter stemming + bigrams, fit-on-corpus, serializable state. **NOT yet ported** (the "already ported" claim was wrong — only the `tfidf_vocabulary` STORAGE repo is; the vectorizer itself is **W4.7e2**, split off from W4.7e). |
