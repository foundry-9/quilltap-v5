...
|---|---|---|---|
| **builtin** (`tfidf-bm25-v1`) | none — **local, in-process** | n/a | `LocalEmbeddingProvider`: TF-IDF + BM25 + Porter stemming + bigrams, fit-on-corpus, serializable state. **PORTED (W4.7e2, 2026-07-07)** — `quilltap-core::tfidf` (stemmer/vectorizer/provider) + `services::builtin_embedding` (host glue) + `services::embedding_refit_job` (the `EMBEDDING_REFIT` handler); `tfidf_vectorizer_equivalence` (tier-1) + `embedding_refit_tier3_equivalence` (tier-3). The IDF `ln` is a ≤1-ULP libm seam vs V8 (persisted `idf` compared numerically at 1e-12). |
...
| **W4.7d** — transport, errors, keys — **DONE** (2026-07-07) | `db::api_keys` (the last unported repo — plaintext-within-encrypted-DB) + `services::api_key_service` (resolvers + user-scoped wrappers + the provider scan; the `ApiKeyResolver` seam closed with a real `ConnApiKeys`, spine wiring → W4.4b); `services::llm_errors` (the 8-class taxonomy + `handleProviderError` + `getUserFriendlyError`); `model::response_parse` (non-streaming parsers, 5 wire families) + `model::provider_models_api` (validate/models); `model::transport` (the trait + pure header/policy helpers, IO-free; feature-gated reqwest impl behind `native-transport`) + `model::completion_provider` (the production composition); the Google `config → wire` framing deferral CLOSED in `build_request`. | `api_keys_tier2_equivalence` (minted-remap) + `llm_errors_equivalence` (54 rows) + `request_builder_google_wire_equivalence` (5 rows byte-exact wire). Non-streaming parsers + validate/models unit-tested (verbatim from v4 source); the recorded-payload differential + the spine ApiKeyResolver wiring are tracked deferrals; transport is integration-smoke (no oracle). |
| **W4.7e** — pricing, capability, logging, embeddings — **sub-units 1–4 DONE** (2026-07-07); sub-unit 5 → **W4.7e2 DONE** (2026-07-07) | ✅ pricing fetcher (`services::pricing_fetcher`: injected `PricingFetch` seam, `now_ms`, 24 h TTL + 5 min negative cache, the two casings, JS `parseFloat`, slug fuzzy match, `findCheapest`, the `estimateMessageCost` cascade — closes the finalizer seam) + `check_model_supports_tools` (backs `model_supports_native_tools`; field removal handed to W4.4b); ✅ `services::llm_logging` (`log_llm_call` writer + `summarize`/`is_logging_enabled`/`map_task_type` + `LogContext` run-id + the `cache_prefix_hashes` leaf); ✅ `model::embedding_wire` (openai / ollama `num_ctx`+404-legacy+NaN-guard / openrouter base64-Float32). **W4.7e3 (the 6 call-site closures) DONE** (2026-07-07): `cheap_llm_exec` (executor-attached `CheapLlmLogConfig` + per-call `task_type`, no spine change), the gatekeeper LLM-classify path (moderation path not ported — projected `ModerationResult` drops the raw per-category `flagged`), `generate_image`/avatar/story (via `generate_with_reroute`), and `primary_stream` (`chunk.done`, requestHashes + finishReason; `durationMs`=0). Proven in-process by `cheap_llm_exec::tests::logging_writes_one_row_through_the_real_writer`. **Follow-ups:** the per-oracle differential regens (un-mock `logLLMCall` + dump `llm_logs`: compression / danger_gatekeeper / answer_confirmation / image_generation / avatar / story / primary_stream / memory_processor / context_summary — `orchestrator_tier3` excluded, no `llm_logs` dump); the interactive-priority throttle is host-timing (nothing ported); **sub-unit 5 (the BUILTIN TF-IDF/BM25 vectorizer) landed as W4.7e2, DONE 2026-07-07** — `quilltap-core::tfidf` + `services::builtin_embedding` + `services::embedding_refit_job`; `tfidf_vectorizer_equivalence` (tier-1) + `embedding_refit_tier3_equivalence` (tier-3); the IDF `ln` is a ≤1-ULP libm seam. | ✅ Pricing/capability: `pricing_fetcher_equivalence` (tier-1, 6 scenarios, v4 real exports, fetch/SDK/repo mocked). ✅ Hashes: `request_prefix_hashes_equivalence` (tier-1, 17). ✅ Embeddings: `embedding_wire_equivalence` (tier-1, 12, v4 real providers). ✅ Call-site writer proof: the in-process `cheap_llm_exec` self-test. Follow-up: the per-oracle `llm_logs`-dump regens. |
