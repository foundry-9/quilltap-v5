...
| B6 | HUMAN | 💸 **the Docker / host-gateway container walk** | One `docker build` on a quiet machine, then the Ollama walk through `host.docker.internal`. | The gateway resolver rewrites the base URL at every provider construction site; the OLLAMA key-test and model-fetch URLs keep their `//api/tags` double slash (v4-faithful — P4.71's flipped pin). **PASS — discharged 2026-09-03, after the walk, once the image built.** The build had OOM'd on the first attempt (Docker Desktop's 14 CPUs against 7.7 GB; fixed by `ARG CARGO_BUILD_JOBS=4`, commit `060ba01f`). Then, rather than a fresh instance needing a passphrase at Setup, the container was pointed at the **dogfood copy itself** (`-v ~/qt-dogfood-friday:/app/quilltap`), which is already provisioned and auto-unlocks — so the application half needed no human after all. **Both halves proven on the wire in one capture.** A `connectionProfileTest` against the instance's REAL Ollama profile (stored `baseUrl: http://localhost:11434`) answered `{"valid":true,…"Successfully connected to OLLAMA"}` from inside a container that has nothing on its own loopback; a negative control on a dead port answered `valid:false`, so the test genuinely reaches out. Pointing a profile at a host listener that logs its request line then showed **exactly** what v5 sends: `REQUEST-LINE: GET //api/tags` with `HOST-HEADER: host.docker.internal:11999` — i.e. the gateway rewrite (P4.71) **and** v4's `//api/tags` double slash, matched byte-for-byte, which is the flipped pin. Finally a real completion: `connectionProfileTestMessage` on `qwen3.5-9b-q6:latest` returned *"Hello! Connection confirmed—everything is working perfectly."* in 8 s. Also confirmed in the boot log: `Docker environment detected — using host.docker.internal as gateway hostname` and `process timezone set timezone=America/Chicago source=QUILLTAP_TIMEZONE`. |
...
| | count |
|---|---|
| Steps PASS | 14 (two with explicitly recorded limits — A5's positive arm, A8's production no-op codec) |
| Steps PARTIAL | 1 (A9 — timing line live, budget not live-observable) |
| Steps DEFERRED-TO-HUMAN | 0 — B6 was discharged after the walk (see its row) |
| v5 defects found | **0** |
| v4 bugs to file | 0 |
| 💸 items discharged | 9 |
...
**💸 still owed:** Pascal's group tier; the Brahma deep-query budget; memory dedup / conversation
