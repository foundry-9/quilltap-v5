...
| A5 | CLAUDE | Open a chat from the 25 with a populated `cycleOrderParticipantIds`; read the participants list order. | The list orders by the persisted rotation (generating seat at the head, latecomers behind), matching the stored id array exactly. | **NOT RUN** — no blocker; ran out of walk before it. |
...
| B2 | CLAUDE | In the same paused room, press **Nudge**, then **Skip**. | Both leave the pause **standing** (`isPaused` stays 1) — the pre-round behaviour lifted it. | **NOT RUN** — Nudge draws a real turn, and the instance default seat (NANOGPT) is 401 here, so the arm would measure the dead key rather than the pause. Wants a seat repointed at DEEPSEEK first. |
...
| C3 | CLAUDE | Investigate the **10 duplicate `generationKey` groups** that have re-accumulated since v4's 2026-09-11 collapse. | Determine whether they are same-character (a missed cache hit — a v4-side question) or legitimately distinct. Record; do not "fix". | **NOT RUN** — the 10 duplicate `generationKey` groups are measured and recorded in §0; deciding whether they are same-character (a missed hit, a v4-side question) needs a join that was not worth the walk time. **Worth an order-sized look.** |
...
| D5 | CLAUDE | LLM Inspector → filter for `VOICE_REWRITE`. | The type appears under the `other` group with its label at all three sites; v4's 17 rows plus D2's are listed. | **NOT RUN** — no blocker; the 20 `VOICE_REWRITE` rows exist for it. |
...
| D8 | CLAUDE | Open the Almanack. | The row **`Impersonated Lines in Character Voice`** reads **Yes** (v4 has it on). | **NOT RUN** — no blocker; one Almanack render away. |
...
| E2 | CLAUDE | Save an image from the gallery into a character vault, then save the **same** image again. | First: `Saved to <mount>` toast. Second: **409** with four **flat** siblings `{error, code, relativePath, keptAt}`, `code = ALREADY_SAVED`. | **NOT RUN** — no blocker; the 409 arm is the interesting half. |
| E3 | CLAUDE | Hit a byte route with `?download=1`. | `Content-Disposition: attachment`. (The Electron `will-download` half is shell-only — not this walk.) | **NOT RUN** — no blocker. |
...
| H2 | CLAUDE | Same endpoint, but send **two chunks** then hold. | `Provider stream went quiet for 120000ms after 2 chunk(s)` — note `chunk(s)`, not `chunks`. | **NOT RUN** — the stalling endpoint supports the `?chunks=2` idle arm and it is written and running; the 120 s budget simply did not fit the remaining walk. **Cheapest unrun row: the server is still up and the endpoint still holds.** |
...
| H4 | CLAUDE | An endpoint that streams a few chunks and then **dies**. | v4 and v5 both **keep the partial** and do NOT substitute; the chain is skipped (`routeVia: primary`, `routeFailures: []`); the turn throws. | **NOT RUN** — needs a socket that dies mid-stream; the endpoint holds rather than dying, so it wants a small change. |
...
| I2 | CLAUDE | Kill the server ungracefully, leaving a stale lock; boot again. | The stale lock is reclaimed (dead PID) without the hostname mattering; the ordered teardown means no orphaned PTY children. | **NOT RUN** — would need an ungraceful kill of the live server mid-walk. |
...
FIXED (#118). 27 rows not run** — 6 honestly deferred to the human (image spend
or a posed provider refusal), 21 simply beyond the walk's length, each with its
reason in the row. No row was left ambiguous.
...
**Cheapest unrun rows, if someone picks this up:** H2 (the 120 s idle-stall arm
— the stalling endpoint already supports `?chunks=2`), D5/D8 (two reads),
E2–E4 (three gallery gestures), A5/A7.

**One thing worth an order:** C3 — **10 duplicate `generationKey` groups have
re-accumulated since v4's own 2026-09-11 collapse**. If those pairs are
same-character, something is missing a cache hit that the cache was built to
catch. Measured, not diagnosed.
