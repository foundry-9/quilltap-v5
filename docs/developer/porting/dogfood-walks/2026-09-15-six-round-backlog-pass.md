...
| C3 | CLAUDE | Investigate the 10 duplicate `generationKey` groups. | **RUN — and it CORRECTS this walk's own earlier claim.** All 10 groups are **same-character** (identical `tags` sets), but **every one of the 20 rows PREDATES v4's 2026-09-11 collapse** — the newest is 2026-09-07, four days before it. So they are **survivors of that collapse, not re-accumulation since it**, and **nothing has duplicated in the four days since** (which is the reassuring half). Why v4's migration left them is a **v4-side question, not a v5 defect**: its `selectAvatarRows` takes every `avatar\_%` IMAGE row with a non-empty `generationPrompt` *regardless of whether it already carries a key*, so these were in scope; the keep-list for portraits a character still names explains only some pairs (some groups have one row referenced by a chat's `characterAvatars`, some none). v5's cache is unaffected — C1 proved it hits. **Candidate upstream filing; the "worth an order" framing has been withdrawn.** | **PASS (no v5 defect)** |
...
**One v4-side question (NOT a v5 defect, and NOT what this walk first
claimed):** C3 — the 10 duplicate `generationKey` groups are all same-character,
but **all 20 rows predate v4's 2026-09-11 collapse** (newest 2026-09-07), so they
are survivors of it rather than re-accumulation, and nothing has duplicated
since. v4's `selectAvatarRows` had them in scope; the portrait keep-list explains
only some pairs. A candidate upstream filing. v5's cache is unaffected — C1
proved it hits.
