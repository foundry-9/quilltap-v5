...
| E6 | CLAUDE | The avatar-preview download rider | Wardrobe → avatar generation pane → download a preview | **PASS — both arms, on a real generated preview.** Preview on Abigail's four-slot outfit (GPT Image 2) produced a real avatar; **Download** gave `download="Abigail_preview.webp"` with a `blob:` href, and `document.querySelectorAll("a[download]").length === 0` — the hidden-anchor mechanism really is retired, which is the whole of the `af1bc479` rider. The catch arm was then forced by making `/api/v1/files/**` answer 404: the toast reads v4's **`Failed to download avatar preview`** (`qt-toast-error`) and the download list stayed at one entry, so a failed fetch saves nothing | PASS |
| E7 | HUMAN | The generate-image download | Generate an image, then Download it | **PASS — a real generation, then a real download.** Generate Image on the default **GPT Image 2** profile with a benign landscape prompt produced one image (`IMAGE_GENERATION` row at 18:05:10, preceded by its `DANGER_CLASSIFICATION`). The hover **Download** (`title="Download"`) fetched the bytes and handed them to `triggerBlobDownload`: `download="generated_1787681089236.webp"` with a `blob:` href — the **file-id source**, not the `generated-image.png` fallback, and no hidden anchor | PASS |
...
| G6 | HUMAN | 💸 The candid story-background arm (P4.D94) | Needs a dangerous-compatible image profile | **PASS — the candid arm proven to the character, on a real dangerous-marked chat.** No profile had to be built: the instance already carries `uncensoredImageProfileId: 6710bbbb-…` (NanoGPT **Chroma**) in its global danger settings and 46 chats flagged `isDangerousChat=1`, so `is_dangerous_chat && has_uncensored_image_provider` was already true. `chatRegenerateBackground` on *Catch-up: Gary, Home Life & Traditions* produced a fresh `IMAGE_PROMPT_CRAFTING` row (101 → 102) whose system prompt measures **4,255 UTF-16 units — exactly** the candid five-element join computed from the constants (the same arithmetic reproduces P4.D94's recorded **5,114** for the concealed variant, which is what validates the method). Both candid markers present (*"The target image provider accepts adult content"*, *"Do NOT reach for coy substitutes"*), both concealment markers absent (*"use cinematic concealment"*, *"- Drapery:"*). ⓘ The image RENDER then failed — see the Grok note in the results log; the crafter's variant selection is what this row tests and it is correct | PASS |
