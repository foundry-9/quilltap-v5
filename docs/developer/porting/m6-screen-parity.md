...
| Agent-mode toggle (sidebar badge) | `components/chat/ChatSidebar.tsx:1116-1127` ("Agent On"/"Agent Off", Chat section), handler `hooks/useChatControls.ts:367-395` | `chat/sidebar/chat-section.ts` | ~~`p4.9e3`~~ **DONE** (P4.9E3C, 2026-07-27). Surfaced by the 2026-07-27 dogfood walk — it is not a modal, so it fell between this table and the `ChatModals.tsx` one below and was tracked in NEITHER. The badge is a TWO-state control over a THREE-state verb and sends only the two arms v4 sends. Placement RESTORED at P4.D141 (2026-09-01): v4 puts it directly after the Concierge control, which was a standing v5 deferral until the four-state control landed in that slot — the badge no longer leads the section |
| Route-trail badge under the avatar | `components/ui/RouteTrailBadge.tsx` (v4 `5841a8c62`) | `chat/route-trail-badge.ts` | **PARITY** (P4.D177) — new in v4 too; v5 lands it the same round |
| Provider/model badge under the avatar | `components/ui/ProviderModelBadge.tsx`, mounted `MessageDesktopAvatar.tsx:35-39` | `chat/sidebar/provider-model-badge.ts`, mounted `chat/message-row.ts` (courier + regular assistant branches) | ~~**MISSING**~~ **PARITY**, closed (P4.D177) — v5's message row previously mounted no badge under the desktop avatar at all (the component existed, used only by the participant card and the add-character dialog) |
| Participants-list rotation (drawn cycle order) | `lib/chat/turn-manager/{cycle-order,turn-order}.ts` (v4 `2aca73ad6`) | `chat/turn-order.ts` (`TurnState.cycleOrder`, step-4 comparator) | **PARITY with a recorded mechanism DIVERGENCE** (P4.D177 §C.2) — v4 recomputes `TurnState` client-side from history via `calculateTurnStateFromHistory`; v5's SPA has no such recompute, so `turnState.cycleOrder` is set from `?action=turn`'s `state.cycleOrder` on every turn response (the ONE live source — a fresh load's `query` refresh resolves and returns it); the chat-GET leg §C.2 named (`parseCycleOrder` over the raw `cycleOrderParticipantIds` string) is DORMANT, since P4.D171 measured that v4's chat GET never projects the key and v5's does not either — never recomputed from message history client-side (matching v5's existing divergence that `spokenSinceUserTurn`/`lastSpeakerId` are server-authoritative only) |
...
| Progressions section (System Prompts tab) | `components/characters/progressions/ProgressionsSection.tsx`, mounted at `system-prompts-editor/index.tsx:91-95` | `progressions/progressions-section.ts`, hosted at `characters/edit/system-prompts-tab.ts` immediately after the subprompts card | **PARITY** (P4.D170, v4 `25f534c0b`) |
| Progression editor modal | `components/characters/progressions/ProgressionEditorModal.tsx` (550) | `progressions/progression-editor-modal.ts` | **PARITY** (P4.D170) — hand-rolled overlay on BOTH sides: v4 does not use its own `Modal` here, so v5 does not use `qt-modal`. The eleven-token placeholder legend and the template placeholder are TS constants, since Angular interpolates a literal `{{…}}`. |
| Workbench `progress` affordances | `components/custom-tools/{GateSection,OutcomesSection,ProvingBench,SideEffectsSection}.tsx` | `screens/custom-tools/{gate-section,outcomes-section,proving-bench,side-effects-section}.ts` | **PARITY** (P4.D170) — the gate subject select, the `Progress…` outcome subject, the `Progress field…` / `Now (epoch ms)` menu items, the `progress.` effect prefix, and the bench's derived list on its own one-second clock |
...
| ImpersonationVoiceDialog | `components/chat/ImpersonationVoiceDialog.tsx`, `ChatModals.tsx:347` | **DONE** (P4.D181, v4 `686954937`) — `chat/impersonation-voice/impersonation-voice-dialog.ts`, mounted FRESH per open in the Salon's modals region, opened by the composer's own submit rather than a button. **Three recorded divergences:** (a) a `Modal`, not v4's `FloatingDialog` — v5 has no draggable/resizable primitive, the sibling rehearsal (Insert Announcement) made the same substitution, and v4's `storageKey="quilltap:impersonation-voice-geometry"` / `initialGeometry` 640×720 / min 420×460 therefore have no counterpart; (b) no Lexical `namespace` on either editor (v5's ProseMirror `qt-markdown-field` needs no instance key, D17); (c) v4's `sendMessage(e: React.FormEvent \| null)` seam — the hook calls the streaming hook with a null event — is a NO-COUNTERPART: v5's composer emits a payload and the Salon's `postComposedMessage` is the single door, so there is no event to null and no `preventDefault` to make; (d) bug 134's `types.ts` consolidation (v4 collapses the salon-local `ChatSettings` + four sub-shapes into re-exports of the settings module) is a NO-COUNTERPART — v5 has ONE declaration, `ChatSettingsDto` in `core-contract.ts`, and never had a second; (e) bug 134's deletion of `useChatData`'s mount-only `fetchChatSettings`/`chatSettings` state is a NO-COUNTERPART — v5's Salon has read settings through the `['chatSettings']` TanStack query since P4.6f, so the staleness the deletion cures never existed here (the facts are pinned structurally in `salon-settings-live.spec.ts`). Rows (d)/(e) added at the `f4ad2c8d1` unification (§3 review). |
| **The composer's clear** | v4's `ChatComposer` clears NOTHING; only `useSSEStreaming.sendMessage` does (`setInput('')` / `setAttachedFiles([])` / `clearDraft()`) | **RESTRUCTURED to match (P4.D181 unit 3)** — v5's `submit()` used to clear the editor, the source text, the send gate, the tray and the persisted draft itself, immediately after `send.emit(...)`. It now emits and stops; `ChatComposer.clearAfterSend()` is public, and the Salon's `postComposedMessage` is the one caller — the direct submit AND both of the dialog's Send doors. An INTERCEPTED submit never calls it, which is what leaves "Edit original" and "Cancel" a draft and a tray to return to. |
| **`VoiceRewriteReviewPanel`** | `components/chat/VoiceRewriteReviewPanel.tsx` (v4 extracted it out of `InsertAnnouncementDialog` in `686954937`) | **DONE** (P4.D181) — `chat/impersonation-voice/voice-rewrite-review-panel.ts`, shared by both rehearsals, extracted mechanically with no behaviour change (the announcement dialog's specs stayed green unchanged). v4's `namespace` prop has no counterpart, per (b) above. |
...
| Capabilities report → **The Almanack** | `components/tools/capabilities-report-dialog.tsx` + `capabilities-report-card.tsx` | `almanack/almanack-report-dialog.ts` + `almanack/almanack-card.ts`, hosted on the Providers tab | **SPA DONE (P4.38, 2026-08-05)** — card + viewer + the shared `qt-progress-bar`; `sectionId` stays `capabilities-report` (the deep-link anchor). Progress rides the ONE global event stream's new `phase` frame, not v4's per-id SSE route (the D3-family divergence). **⚠ server OPEN under P4.37** (collectors/verbs/host wire HELD at the `f7f1a956` unification pending their tier-2 differential — the card answers the empty state until the resumed lane lands) |
...
| HelpChatDialog | `components/help-chat/HelpChatDialog.tsx`, mounted `app-layout.tsx:135` | `help/help-dialog.ts` hosted by `help/help-entry.ts`, mounted `shell.ts` before the Brahma entry | **PARITY** (P4.9I2B, unified 2026-09-05; the centred overlay in place of `FloatingDialog` recorded) |
...
| `roleplayTemplateId` toolbar awareness (`status-log.md:12226-12227`; re-scoped `phase-4.md:1615-1618`) | LANDED (P4.9L, 2026-08-14) — §4 row 14 |
...
| Files: rich text/pdf preview, drag relocation, workspace-tab drill (`p4.6af:16-18`) — the rich FolderPicker LANDED at P4.D147 (v4 bug 113) | §4 `p4.9n` |
...
| 11 | `p4.9i` → **`p4.9i1` + `p4.9i2`** | BrahmaConsoleDialog (+ its `asTab` re-skin) ∥ HelpChatDialog. **SPLIT 2026-07-18** — two surfaces, two backends, ~2,500 LOC (see the §1.6 correction). `p4.9i1` = the unported multi-turn `orchestrator.service.ts` + chat-CRUD dispatch + the dialog (and it must carry `HelpChatComposer` + the `qt-help-*` styles, which Brahma depends on). `p4.9i2` = the 9-component help family + read verbs for the help docs `help_doc_sync.rs` already writes. ⚠ P4.d11 bank note (2026-07-20): v4 REWROTE `help/math-notation.md` at `5915b04e` (the "discreet doorman" copy, alongside the single-dollar promotion) — the eventual help port copies the NEW text, not the b8b12695-era file already banked at P4.d9 | lane each | `p4.9i1`: W4.5b one-shot engine (landed). `p4.9i2`: nothing above `help_doc_sync.rs` ⚠ **P4.D77 bank note (2026-08-14, v4 `24633026`)** — the server half of v4's section-level help search IS ported (the `help_doc_chunks` table + repo, the chunking, the sync re-slice and upgrade backfill, the HELP_DOC job's chunk pass, the reindex/reapply riders, and `help_search`'s section ranking + `matchedSection` block). Three halves of that commit were deliberately left here, surveyed verbatim so this lane does not re-derive them: **(a) `GET /api/v1/help-docs?action=search&q=`** — a plain case-insensitive SUBSTRING match over title+content (explicitly NOT the semantic tool: no stemming, no ranking beyond title-hits-first), returning `{matches: [{slug, titleHit, snippet}]}`; `q.trim().length < 2` returns an empty list; the snippet comes from `buildSnippet` with the **deliberately lopsided** `SNIPPET_LEAD 30` / `SNIPPET_TRAIL 160` (v4's why: the snippet renders on one truncated line, so a balanced window clips the matched word off the right-hand end — a short run-up puts it near the start where it survives truncation), Markdown furniture stripped (` ``` ` → space, then `[*_`#|>]` dropped, then whitespace collapsed) and a `…` prepended/appended when the slice is interior; the sort is `Number(b.titleHit) - Number(a.titleHit)`. **(b) `HelpGuideTab.tsx`** — a 200 ms `SEARCH_DEBOUNCE_MS` before the fetch, the instant client-side TITLE filter kept so the list narrows on the first keystroke rather than waiting on a round trip, and results TAGGED with the query that produced them (`{query, bySlug}`) so a stale response cannot leak into a newer filter; `HelpCategorySection.tsx` gains an optional `snippets` map and renders the snippet as a muted `text-xs` line under the topic title, adding `whitespace-normal` to that button. **(c) the help COPY deltas** — `help/chat-settings.md`'s image-description section was rewritten (it named a dropdown that no longer exists, claimed the blank option disabled descriptions when it auto-selects, and never mentioned the uncensored fallback profile) and `help/help-chat.md` got a one-line change; the P4.D73 precedent applies — copy the NEW text when the help family lands, not the older file. **⚠ P4.D93 bank note (2026-08-19, v4 `9125f492`, bug 81)** — HelpChat is the FOURTH of v4's four api-key-lookup call sites and the one with no v5 target, so it is banked here verbatim. v4's `lib/services/help-chat/orchestrator.service.ts:211-221` now reads `const keyResolution = await resolveConnectionProfileApiKey(repos, connectionProfile)` and, on `!keyResolution.ok`, throws `keyResolution.reason === 'no-api-key-configured' ? 'No API key configured' : 'API key not found'` — note the SHORTER first sentence, which differs from both Brahma sites (the one-shot service's lower-case `no API key configured for this connection profile` and the orchestrator's capitalised one). The v5 twin to call is `services::api_key_service::resolve_connection_profile_api_key(conn, provider, api_key_id)`, which returns `ProfileApiKeyResolution::Ok(key)` / `Failed(NoApiKeyConfigured)` / `Failed(ApiKeyNotFound)`; the two capability predicates live beside it. Port the sentence bytes per-site, do not normalize the casing. **⚠ P4.D95 bank note (2026-08-20, v4 `870a57fa`)** — v4 added a new `## Consulting the Shelf of Past Conversations` section (5 paragraphs, ~12 lines) to `help/memory-recall-relevance.md`, sitting between the expand-related section and `## What Else Recall Quietly Does`. It documents the per-turn conversation-summary cadence this lane ported (the setting, the three original cadences, the reused embedding, the two dedup courtesies + the recap stand-down, the 3→10 ramp, and the instance-wide-no-override rule). The P4.D73/P4.D77 precedent applies: copy the NEW file when the help family lands, not the older banked one. **⚠ P4.D106 bank note (2026-08-23, v4 `a14a1811`, bugs 91+93)** — two help files grew sections documenting surfaces this lane ported: `help/connection-profiles.md` gained a "Two questions, not one" section (the model-sees vs connector-sends distinction behind `providerCanTransportImages`, the three-outcome table, and a sentence appended to the Ollama/OpenAI-Compatible tick guidance) and `help/dangerous-content.md` gained "When a Provider Refuses Outright" (the moderation finish-reason recogniser and why "try resending" was retired for a named refusal). The P4.D73 precedent applies: copy the NEW files when the help family lands. **⚠ P4.D108 bank note (2026-08-23, v4 `a14a1811`, bug 92)** — v4 rewrote `help/keep-image-tools.md` for the new `describe_image` tool: the H1 retitled to "The Photo Album — Looking At, Keeping, Listing, and Re-Attaching Images"; the intro now names FOUR instruments and adds a which-is-which paragraph ("Exactly one of them — **`describe_image`** — tells a character what a picture *contains*…"); a new `### describe_image(uuid)` subsection (three numbered serve tiers, the no-album rule "looking at a thing is not the same as owning it", the vision-capable caveat); the `attach_image` section gains "**This does not let the character see the picture.**…wants `describe_image`"; and the how-the-character-knows-the-UUID paragraph now routes to `describe_image` (to find out what it shows) or `keep_image` (to file it). The P4.D73/P4.D77 precedent applies: copy the NEW file when the help family lands. **DISCHARGED (2026-09-05): `p4.9i2` landed whole at the `p4.9i2` round (P4.9I2A server ∥ P4.9I2B SPA); every bank note in this row is present in the vendored `help/` tree at `d883a5ee1` (measured in the P4.9I2A lane record), and the P4.D77 half (b) snippet line is `help-category-section.ts`.** |
...
| 13 | `p4.9n-files-fidelity` | rich text/pdf preview, cross-mount move/copy UI, drag relocation (the rich FolderPicker landed at P4.D147) | lane | pdf/docx extractor (refusing seam) |
| 14 | ~~`p4.9l-salon-composer-toolbar`~~ | **DONE 2026-08-14 (P4.9L).** The composer LAYOUT is v4's (2-column gutter grid + composer-toggle stack + toolbar row + dominant editor); the `#75` band-aid (`.qt-chat-composer-actions { flex-basis: 100% }`) is retired and the e2e guard rewritten to the real geometry. The formatting toolbar mounts in composition mode with the markdown buttons, the indent controls, the code-block toggle, BOTH character pickers (their composer entrance, deferred by name since P4.D75), the source toggle, and the `roleplayTemplateId`-aware delimiter buttons — the last of these ported against a new v4-side oracle. Divergences recorded in the lane record: Continue takes the gutter grid's free cell (v4 has no composer Continue at all), and a send made from source mode carries the SOURCE bytes where v4 silently discards them | lane | ~~a composer toolbar must exist first~~ (the shared field toolbar was it) |
| 14b | ~~`p4.9l2-document-pane-toolbar`~~ | **DONE 2026-08-20 (P4.9L2).** The pane mounts the SAME `qt-formatting-toolbar` the composer does, in v4's `.qt-doc-toolbar` row above the editor (`DocumentPane.tsx:323-355, :686-693`), with the whole markdown inventory, both pickers, the source toggle and the template's delimiter buttons — and NO `narrationDelimiters`, so no synthesized "Nar" button, pinned against the P4.9L recorded vectors. The three wirings the row named are done: the source branch acts on THIS pane's textarea through its frontmatter-recombine seam, the toolbar's source toggle shares the header button's `showSource` through one `toggleSourceMode()` (which flushes on the way out, as v4's does), and the Salon threads the template it already fetched into the pane while the standalone view deliberately passes nothing (v4 `StandaloneDocumentView.tsx:381`). Live in `salon-documents-flow` + `workspace-document-standalone-flow`. **One divergence recorded:** v5's header source button has no v4 counterpart — it was added when this pane had no toolbar — and is kept sharing the one signal, a candidate for retirement | rider | ~~a composer toolbar must exist first~~ (landed, P4.9L) |
