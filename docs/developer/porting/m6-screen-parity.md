...
| 14b | `p4.9l2-document-pane-toolbar` | the Document-Mode pane's formatting toolbar. v4's `DocumentPane` mounts the SAME `FormattingToolbar` the composer does, in a `.qt-doc-toolbar` row (`DocumentPane.tsx:44, :343-355`) — with `roleplayTemplateId` but NO `narrationDelimiters`, so it shows the template's delimiters and no synthesized "Nar" button. P4.9L surveyed this mount and left it: every component exists, but the pane's own wiring does not (the source branch must act on THIS textarea; the toolbar's source toggle must share the header button's `showSource` rather than open a second one; `roleplayTemplateId` must reach the pane, which the Salon does not pass it today). Its live beats are `salon-documents-flow` + `workspace-document-standalone-flow` | rider | ~~a composer toolbar must exist first~~ (landed, P4.9L) |
