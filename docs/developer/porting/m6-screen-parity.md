...
| LibraryFilePickerModal | `ChatModals.tsx:246` | **DEFERRED BY NAME (P4.9E3C).** 616 LOC over SIX endpoints (projects, mount points, `?action=photo-albums`, `?action=group-stores`, `files?action=attach-mount-file`, `files?action=link`) plus character/user photos; and `attach-mount-file` is P4.9E3B's own tier-2 deferral. The composer-gutter entry is ABSENT rather than refusing. Its own round. |
