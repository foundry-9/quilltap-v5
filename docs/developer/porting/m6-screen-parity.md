...
| Participants-list rotation (drawn cycle order) | `lib/chat/turn-manager/{cycle-order,turn-order}.ts` (v4 `2aca73ad6`) | `chat/turn-order.ts` (`TurnState.cycleOrder`, step-4 comparator) | **PARITY with a recorded mechanism DIVERGENCE** (P4.D177 §C.2) — v4 recomputes `TurnState` client-side from history via `calculateTurnStateFromHistory`; v5's SPA has no such recompute, so `turnState.cycleOrder` is set directly from the chat GET's raw `cycleOrderParticipantIds` string (parsed by `parseCycleOrder`) and refreshed from `?action=turn`'s `state.cycleOrder` on every turn response — never recomputed from message history client-side (matching v5's existing divergence that `spokenSinceUserTurn`/`lastSpeakerId` are server-authoritative only) |
