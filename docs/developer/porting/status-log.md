...
### Units 2–3 — the `quilltap-tauri` crate: scaffold + boot + §1 + §2 + §3
### + the IPC contract suite (2026-07-15)

`crates/quilltap-tauri` at 0.0.1 — **tauri 2.11.5 / tauri-build 2.6.3 /
wry 0.55.1 / tao 0.35.3** (latest 2.x stable at lane start; tauri-cli
2.11.4 via `cargo install tauri-cli --locked`). The workspace glob picked
the crate up; the core's default-build purity untouched.

- **Scaffold**: `tauri.conf.json` (identifier `com.quilltap.desktop`,
  window 1280×800, CSP null dev-grade, bundle targets `["app"]`),
  `build.rs` = `tauri_build::build()` behind a dist-materializer (the
  Angular dist — verified `apps/web/dist/quilltap/browser`, angular.json's
  project is `quilltap`, NOT the order's expected `dist/web/browser` — is
  gitignored, so `build.rs` creates it EMPTY when absent to keep a fresh
  checkout's `cargo build` green; a real `ng build` fills the same path).
  Dev-grade icons generated by `cargo tauri icon` from a placeholder PNG
  (android/ios sets pruned — D21 defers mobile).
- **Boot**: `resolve_instance_base_dir` + `production_host_config` +
  `boot_startup_status` (the unit-1 lib helpers) inside
  `tauri::async_runtime::block_on`, so the Host's cadence tasks land on
  the process-lifetime runtime. Unknown process args ignored (macOS
  `-psn_…`), `--data-dir`/`--instance` accepted.
- **§1**: `dispatch` (arg key `request`) ALWAYS resolves the envelope via
  the reused `dispatch_body` (boot-failure + malformed arms included);
  `health` returns `{status, body}` from `health_parts`.
- **§2**: `events_attach` (idempotent — a re-attach aborts the previous
  forwarder under the slot lock) → `quilltap://event` frames, payload =
  the SSE `data:` serialization exactly; `RecvError::Lagged` →
  `quilltap://resync`, keep forwarding. Ordering through
  `subscribe_with_backlog` — subscribe-before-snapshot preserved.
- **§3**: `register_asynchronous_uri_scheme_protocol("qtap", …)` →
  `protocol::handle_qtap_request` — tower `oneshot` into the SAME
  `build_router` over the SAME booted state, body buffered, ACAO `*`
  stamped on every response, OPTIONS preflight answered 204 without
  touching the router.
- **The tier-4 #2 suite** (`tests/ipc_contract.rs`, 5 tests): mirrors
  `contract.rs` case-for-case — health vocabulary (200/423/503 as
  `{status,…}`), dispatch health round trip, malformed → BadRequest
  envelope, chatSend-no-driver → internal envelope, the locked-vault +
  setup-flow arcs (Locked envelope with v4's merged setup body), the
  boot-failure arm, the ordered §2 trace (Green-Room backlog BEFORE live,
  byte-equal payloads, and the re-attach REPLAYS the still-active backlog
  — the D6 rule, same as an SSE reopen), §3 GET `/health` + `/` + `/setup`
  + CORS preflight. **Mechanism** (the order's allowed fallback): the
  command cores driven directly over a real booted `StartupStatus`, a
  `mock_builder` app supplying the real emit→listen system, PLUS one
  `get_ipc_response` case pinning the command-name/arg-key wiring.
  Gotcha worth keeping: Tauri 2's ACL only skips app commands from a
  LOCAL origin — the invoke test must use `tauri://localhost` on macOS
  (`http://tauri.localhost` is remote there → "not allowed. Plugin not
  found"). Test support duplicated from `quilltap-web/tests/common`
  (bare/fixture materializers; fixture FILES read from quilltap-web's
