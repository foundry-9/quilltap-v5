/**
 * Tier-2 oracle case — the `terminal_sessions` repo (Phase-2).
 *
 ...
 * diff). Banks a clean strings-plus-nullables shape (NO boolean, NO JSON
 * column): three nullable string columns (`label`, `exitedAt`,
 * `transcriptPath`) and one nullable REAL-affinity unbounded-int column
 * (`exitCode` — int with no .max() maps to REAL; an integer-valued cell like 137
 * dumps back as the JSON integer 137 via js_number_to_json).
 *
 ...
 *      sequence from `terminal-sessions-tier2.json` via the real
 *      `TerminalSessionsRepository`;
 *   3. close, then dump the table canonically (RAW on-disk cells via `rawQuery`:
 *      nulls explicit, exitCode as the on-disk REAL/integer) and emit it as one
 *      NDJSON row.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'terminal-sessions-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_TS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_TS must point at the seed fixture from build-terminal-sessions-fixture.ts'
    );
  }

  // Work on a fresh copy so the shared seed fixture stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-ts-oracle-'));
  // v4 nests working files under `<dataDir>/data/` (instance lock, sibling DBs).
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'ts-work.db');
  ...
  // exitCode as the on-disk REAL/integer).
  const columns = (
    (await rawQuery('PRAGMA table_info(terminal_sessions)')) as Array<{
      name: string;
    }>
  ).map((c) => c.name);
  const rawRows = (await rawQuery('SELECT * FROM terminal_sessions')) as Array<
  ...
  process.stdout.write(
    JSON.stringify({ case: 'terminal-sessions-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`terminal-sessions-tier2 oracle failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
