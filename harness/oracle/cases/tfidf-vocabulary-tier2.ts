/**
 * Tier-2 oracle case — the `tfidf_vocabulary` repo (Phase-2).
 *
 ...
 * diff). Widens the tier-2 surface with PLAIN-STRING columns that hold JSON text
 * (`vocabulary`, `idf` — z.string(), stored as-is, NOT re-stringified) and two
 * REAL number columns (`avgDocLength` bare z.number(); `vocabularySize`
 * z.number().int().positive() — min only, still REAL). A fractional avgDocLength
 * dumps as a float; integer-valued REALs dump as JSON integers.
 *
 ...
 *      sequence from `tfidf-vocabulary-tier2.json` via the real
 *      `TfidfVocabularyRepository`;
 *   3. close, then dump the table canonically (RAW on-disk cells via `rawQuery`:
 *      booleans as 0/1, REAL number columns as numbers, plain JSON-text strings
 *      single-encoded, nulls explicit) and emit it as one NDJSON row.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'tfidf-vocabulary-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_TV;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_TV must point at the seed fixture from build-tfidf-vocabulary-fixture.ts'
    );
  }

  // Work on a fresh copy so the shared seed fixture stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-tv-oracle-'));
  // v4 nests working files under `<dataDir>/data/` (instance lock, sibling DBs).
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'tv-work.db');
  ...
  // REAL number columns as numbers, vocabulary/idf as the single-encoded JSON
  // text).
  const columns = (
  ...
  process.stdout.write(
    JSON.stringify({ case: 'tfidf-vocabulary-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`tfidf-vocabulary-tier2 oracle failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
