/**
 * Tier-2 oracle case — the `embedding_profiles` repo (Phase-2).
 *
 ...
 * diff). Banks the Taggable lineage (`userId` + a JSON `tags` array), two
 * nullable REAL number columns (`dimensions`, `truncateToDimensions`), two
 * boolean columns (`normalizeL2`, `isDefault`), two nullable string columns
 * (`apiKeyId`, `baseUrl`), and an enum TEXT column (`provider`).
 *
 ...
 *      sequence from `embedding-profiles-tier2.json` via the real
 *      `EmbeddingProfilesRepository`;
 *   3. close, then dump the table canonically (RAW on-disk cells via `rawQuery`:
 *      booleans as 0/1, integer-valued REALs as JSON integers, `tags` as the
 *      compact JSON text, nulls explicit) and emit it as one NDJSON row.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'embedding-profiles-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_EP;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_EP must point at the seed fixture from build-embedding-profiles-fixture.ts'
    );
  }

  // Work on a fresh copy so the shared seed fixture stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-ep-oracle-'));
  // v4 nests working files under `<dataDir>/data/` (instance lock, sibling DBs).
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'ep-work.db');
  ...
  const columns = (
    (await rawQuery('PRAGMA table_info(embedding_profiles)')) as Array<{
      name: string;
    }>
  ).map((c) => c.name);
  const rawRows = (await rawQuery('SELECT * FROM embedding_profiles')) as Array<
...
main().catch((err) => {
  process.stderr.write(
    `embedding-profiles-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
