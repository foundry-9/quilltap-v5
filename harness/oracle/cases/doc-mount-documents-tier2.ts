/**
 * Tier-2 oracle case — the `doc_mount_documents` repo (a content-text
 ...
 *   - ops run through the REAL `DocMountDocumentsRepository` (overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'doc-mount-documents-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_DOC_MOUNT_DOCUMENTS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_DOC_MOUNT_DOCUMENTS must point at the seed fixture from build-doc-mount-documents-fixture.ts'
    );
  }

  // Work on a fresh copy of the mount-index fixture so the shared seed stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-dmd-oracle-'));
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'dmd-mount-index-work.db');
  ...
  const columns = (
    midb.pragma('table_info(doc_mount_documents)') as Array<{ name: string }>
  ).map((c) => c.name);
  const rawRows = midb
    .prepare('SELECT * FROM doc_mount_documents')
  ...
  process.stdout.write(
    JSON.stringify({ case: 'doc-mount-documents-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`doc-mount-documents-tier2 oracle failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
