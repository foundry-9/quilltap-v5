/**
 * Tier-2 oracle case — the `doc_mount_folders` repo (a mount-index sibling-DB
 * repo of Phase 2, after the pilot `group_character_members`).
 *
 ...
 *   - ops run through the REAL `DocMountFoldersRepository` (overridden
 ...
 *
 * This corpus banks the nullable-UUID `parentId` column: the root folder dumps
 * `parentId: null`, the children dump it as a UUID.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'doc-mount-folders-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_DOC_MOUNT_FOLDERS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_DOC_MOUNT_FOLDERS must point at the seed fixture from build-doc-mount-folders-fixture.ts'
    );
  }
  ...
  const columns = (
    midb.pragma('table_info(doc_mount_folders)') as Array<{ name: string }>
  ).map((c) => c.name);
  const rawRows = midb
    .prepare('SELECT * FROM doc_mount_folders')
  ...
  process.stdout.write(
    JSON.stringify({ case: 'doc-mount-folders-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(
    `doc-mount-folders-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
