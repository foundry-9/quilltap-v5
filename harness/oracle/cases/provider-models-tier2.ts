/**
 * Tier-2 oracle case #5 — the `provider_models` repo (Phase-2, repo #5 after
 ...
 * diff). Widens the tier-2 surface with two NULLABLE REAL number columns
 * (`contextWindow`, `maxOutputTokens`), two boolean columns (`deprecated`,
 * `experimental`), a nullable string column (`baseUrl`), and enum TEXT columns
 * (`provider`, `modelType`). System-wide: no `userId`. Scope is create / update
 * / delete only (no upsert/find helpers, no conflict/built-in behavior).
 *
 * Flow (mirrors text-replacement-rules-tier2.ts, MINUS the expectThrow path):
 *   1. copy the SEED-ONLY fixture to a fresh working copy;
 *   2. open the copy through v4's real `initializeDatabase()` and run the op
 *      sequence from `provider-models-tier2.json` via the real
 *      `ProviderModelsRepository`;
 *   3. close, then dump the table canonically (RAW on-disk cells via `rawQuery`:
 *      booleans as 0/1, an integer-valued REAL contextWindow as the integer) and
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'provider-models-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_PM;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_PM must point at the seed fixture from build-provider-models-fixture.ts'
    );
  }

  // Work on a fresh copy so the shared seed fixture stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-pm-oracle-'));
  // v4 nests working files under `<dataDir>/data/` (instance lock, sibling DBs).
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'pm-work.db');
  ...
  // integer-valued contextWindow as the integer).
  const columns = (
  ...
  process.stdout.write(
    JSON.stringify({ case: 'provider-models-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(
    `provider-models-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
