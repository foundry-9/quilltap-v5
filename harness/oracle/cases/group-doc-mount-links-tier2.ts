/**
 * Tier-2 oracle case — the `group_doc_mount_links` repo (a mount-index
 ...
 *   - ops run through the REAL `GroupDocMountLinksRepository` (overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'group-doc-mount-links-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_GROUP_DOC_MOUNT_LINKS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_GROUP_DOC_MOUNT_LINKS must point at the seed fixture from build-group-doc-mount-links-fixture.ts'
    );
  }

  // Work on a fresh copy of the mount-index fixture so the shared seed stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-gdml-oracle-'));
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'gdml-mount-index-work.db');
  ...
  const columns = (
    midb.pragma('table_info(group_doc_mount_links)') as Array<{ name: string }>
  ).map((c) => c.name);
  const rawRows = midb
    .prepare('SELECT * FROM group_doc_mount_links')
  ...
  process.stdout.write(
    JSON.stringify({ case: 'group-doc-mount-links-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(
    `group-doc-mount-links-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
