/**
 * Tier-2 oracle case — the `project_doc_mount_links` repo (a mount-index
 ...
 *   - ops run through the REAL `ProjectDocMountLinksRepository` (overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'project-doc-mount-links-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_PROJECT_DOC_MOUNT_LINKS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_PROJECT_DOC_MOUNT_LINKS must point at the seed fixture from build-project-doc-mount-links-fixture.ts'
    );
  }
  ...
  const { ProjectDocMountLinksRepository } = await import(
    '@/lib/database/repositories/project-doc-mount-links.repository'
  );

  await initializeDatabase();
  const repo = new ProjectDocMountLinksRepository();
  ...
  const columns = (
    midb.pragma('table_info(project_doc_mount_links)') as Array<{ name: string }>
  ).map((c) => c.name);
  const rawRows = midb
    .prepare('SELECT * FROM project_doc_mount_links')
  ...
  process.stdout.write(
    JSON.stringify({ case: 'project-doc-mount-links-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(
    `project-doc-mount-links-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
