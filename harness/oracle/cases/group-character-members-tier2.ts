/**
 * Tier-2 oracle case — the `group_character_members` repo (the FIRST mount-index
 * sibling-DB repo of Phase 2).
 *
 ...
 *
 * ── THE SIBLING-DB MACHINERY (what's new here) ───────────────────────────────
 ...
 *   - ops run through the REAL `GroupCharacterMembersRepository` (overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'group-character-members-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_GROUP_CHARACTER_MEMBERS;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_GROUP_CHARACTER_MEMBERS must point at the seed fixture from build-group-character-members-fixture.ts'
    );
  }
  ...
  const repo = new GroupCharacterMembersRepository();
  ...
  const columns = (
    midb.pragma('table_info(group_character_members)') as Array<{ name: string }>
  ).map((c) => c.name);
  const rawRows = midb
    .prepare('SELECT * FROM group_character_members')
  ...
  process.stdout.write(
    JSON.stringify({ case: 'group-character-members-tier2', ...dump }) + '\n'
  );
  process.exit(0);
}
...
main().catch((err) => {
  process.stderr.write(
    `group-character-members-tier2 oracle failed: ${err?.stack ?? err}\n`
  );
  process.exit(1);
});
