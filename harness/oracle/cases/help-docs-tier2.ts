/**
 * Tier-2 oracle case #5 — the `help_docs` repo (Phase-2, repo #5 after
 ...
 * diff). THE HEADLINE: this is the first tier-2 case with a BLOB column — the
 * `embedding` Float32 buffer — exercised on insert (number[] -> BLOB), as NULL
 * (null -> NULL), and through a text-only update that must LEAVE the embedding
 * BLOB untouched (v4's whole-row rewrite re-persists it; see the spec comment).
 *
 ...
 *      sequence from `help-docs-tier2.json` via the real `HelpDocsRepository`;
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, '..', 'fixtures', 'help-docs-tier2.json');
  const spec = JSON.parse(readFileSync(specPath, 'utf8')) as Spec;

  const fixture = process.env.QT_FIXTURE_HD;
  if (!fixture || !existsSync(fixture)) {
    throw new Error(
      'QT_FIXTURE_HD must point at the seed fixture from build-help-docs-fixture.ts'
    );
  }

  // Work on a fresh copy so the shared seed fixture stays pristine.
  const scratch = mkdtempSync(join(tmpdir(), 'qt-hd-oracle-'));
  // v4 nests working files under `<dataDir>/data/` (instance lock, sibling DBs).
  mkdirSync(join(scratch, 'data'), { recursive: true });
  const work = join(scratch, 'hd-work.db');
  ...
  process.stdout.write(JSON.stringify({ case: 'help-docs-tier2', ...dump }) + '\n');
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`help-docs-tier2 oracle failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
