 ...
 *   5. P4.D198 (v4 bug 151, `bcd7e4852`): the corpus's `extraMounts` blobs, one
 *      per bug-151 mount-path row, their minted link ids echoed to the sidecar
 *      under `extraMountLinkIds`. A file or blob declaring `fsmBytesPattern` /
 *      `dataPattern` gets GENERATED bytes (`(i * 7 + 13) & 0xff`) rather than a
 *      literal array — the shrink rows need buffers a JSON array has no
 *      business carrying — and `files.size` follows the generated length, since
 *      the mount path's post-processing `size` comparand would otherwise lie.
 *
...
interface FileSpec {
  ...
  fsmBytesUtf8?: string;
  /**
   * P4.D198: generated `(i * 7 + 13) & 0xff` bytes instead of a literal array.
   * The bug-151 rows need buffers a JSON array has no business carrying, and
   * the same generator runs on the Rust side (`original_bytes`).
   */
...
interface Spec {
  ...
  /**
   * P4.D198: further blobs on the SAME mount point. v4's bug-151 hunk calls the
   * mount path "where every character avatar actually travels", so the shrink's
   * primary arm needs its own blob rather than sharing the pre-existing note's.
   * Each minted link id is echoed to the `.meta.json` sidecar under its key.
   */
  extraMounts?: Array<{
    key: string;
    relativePath: string;
...
/**
 * P4.D198's generated buffer: `(i * 7 + 13) & 0xff`. Mirrored byte for byte by
 * `harness/oracle/lib/shrink-script.ts`'s `originalBuffer` and by the Rust
 * `original_bytes`, so the scripted encoder can key on these exact bytes.
 */
function patternBuffer(len: number): Buffer {
...
async function main(): Promise<void> {
    ...
    await repos.files.create(
      {
        ...
        mimeType: f.mimeType,
        // The `size` column must agree with the bytes the FSM seam serves, or
        // the mount path's post-processing `size` comparand lies.
        size:
          f.fsmBytesPattern?.len ??
          f.fsmBytes?.length ??
  ...
  // 6. P4.D198 — the extra blobs the bug-151 mount-path rows load.
  const extraMountLinkIds: Record<string, string> = {};
  for (const m of spec.extraMounts ?? []) {
    const data = patternBuffer(m.dataPattern.len);
    const extra = await repos.docMountFileLinks.linkBlobContent({
      mountPointId: spec.mount.mountPointId,
      relativePath: m.relativePath,
      fileName: m.fileName,
      folderId: null,
      originalFileName: m.originalFileName ?? m.fileName,
      originalMimeType: m.storedMimeType,
      storedMimeType: m.storedMimeType,
  ...
  process.stderr.write(
    `built file-attachment fixtures: main=${mainOut} mount=${mountOut} ` +
      `mountLinkId=${linkResult.link.id} extraMounts=${JSON.stringify(extraMountLinkIds)}\n`,
  );
  process.exit(0);
}
