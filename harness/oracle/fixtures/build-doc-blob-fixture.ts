/**
 * Fixture builder for the doc-edit BLOB tool-handler differential
 * (W4.1d batch 3b, the blob half).
 *
 * Bakes the shared starting state across TWO databases (main + mount-index) so
 * BOTH v4 and the Rust port drive the same doc_*_blob tool handlers against a copy
 * of byte-identical store state:
 ...
 *
 * No blob rows are seeded — the corpus's write_blob ops create them. The Librarian
 * / reindex / transcode side effects the handlers fire are documented no-op /
 * passthrough seams (mocked in the oracle case).
 *
 * Run (Node 24, from the v4 checkout):
 *   N=~/.nvm/versions/node/v24.13.1/bin
 *   cd ~/source/quilltap-server
 *   QT_FIXTURE_DBLOB_MAIN=/tmp/qt-dblob-main.db QT_FIXTURE_DBLOB_MOUNT=/tmp/qt-dblob-mount.db \
 *     $N/npx tsx ~/source/quilltap-v5/harness/oracle/fixtures/build-doc-blob-fixture.ts
 */
...
async function main(): Promise<void> {
  ...
  const spec = JSON.parse(readFileSync(join(here, 'doc-blob.json'), 'utf8')) as Spec;
  const TS = spec.seedTimestamp;

  const mainOut = process.env.QT_FIXTURE_DBLOB_MAIN;
  const mountOut = process.env.QT_FIXTURE_DBLOB_MOUNT;
  if (!mainOut || !mountOut) {
    throw new Error('QT_FIXTURE_DBLOB_MAIN and QT_FIXTURE_DBLOB_MOUNT must both point at the .db files to write');
  }
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-dblob-fixture-build-'));
  ...
  // The blob byte-store table is hand-written (its `data BLOB` column is absent
  // from the metadata schema) — materialize it via the repo's own DDL so the
  // fixture's shape matches production before any create lands.
  midb.exec(`
  ...
  await repos.users.create(
    { username: 'blob-tester', name: 'Blob Tester' } as never,
  ...
  process.stderr.write(
    `built doc-blob fixtures: main=${mainOut} mount=${mountOut} (V1=${v1}, OS1=${os1})\n`,
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`doc-blob fixture build failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
