/**
 * P4.D155 — bring the committed `pascal-run-custom-{main,mount}.db` pair up to
 * v4's current schema vintage (measured at v4 `d883a5ee1`).
 *
 * ## Why this exists
 *
 * Two of the lane's own §A families — `pascal_custom_tools_route_equivalence`
 * and `pascal_run_custom_handler_equivalence` — could no longer REGENERATE
 * their oracle: v4's `BaseRepository` writes the WHOLE validated entity, so a
 * column the fixture predates is fatal, and both cases seed a connection
 * profile. The regen died on `table connection_profiles has no column named
 * allowTierFallback` — the P4.D135 fallback-chain columns, which no pascal
 * family had regenerated across since. The same P4.52 vintage class, on a
 * different pair, and the reason a family that is never regenerated rots
 * invisibly.
 *
 * ## The measured gap (fixture schema vs `fresh_schema.json` at `d883a5ee1`)
 *
 ...
 *   connection_profiles  multiCharacterPrefill, fallbackProfileId, allowTierFallback
 *
 ...
 * MANAGED_FIELDS — v4 `delete`s them from the DB row on every create AND
 * update, the vault files are their sole source of truth, and NO v4 migration
 * ever adds them. P4.52 recorded that ruling for the memories pair; it holds
 * here for the same reason.
 *
 * ## The DDL is v4's own
 *
 * Each statement below is v4's real migration SQL, verbatim, behind v4's own
 * "only if the column is missing" guard — the `migrate-memories-fixture-columns`
 ...
 *
 * The one recorded disagreement carries over verbatim: `generateDDL` emits
 * `multiCharacterPrefill` as a bare `INTEGER` while v4's migration adds it
 * `INTEGER DEFAULT 1` (P4.D79's two-D23-shapes finding). The migration wins.
 *
 ...
 * the .db files named EXPLICITLY — this lane widens the pascal pair only:
 *   N=~/.nvm/versions/node/v24.13.1/bin
 *   W=<this worktree>
 *   cd ~/source/quilltap-server
 *   $N/node --import tsx $W/harness/oracle/fixtures/migrate-pascal-run-custom-columns.ts \
 *     $W/crates/quilltap-web/tests/fixtures/pascal-run-custom-main.db \
 *     $W/crates/quilltap-web/tests/fixtures/pascal-run-custom-mount.db
 */
...
const MIGRATIONS: { table: string; column: string; sql: string; source: string }[] = [
  ...
  },
  {
    ...
    source: 'migrations/scripts/add-profile-fallback-fields.ts:31',
  },
  {
    table: 'connection_profiles',
    column: 'allowTierFallback',
    sql: 'ALTER TABLE "connection_profiles" ADD COLUMN "allowTierFallback" INTEGER DEFAULT 0',
    source: 'migrations/scripts/add-profile-fallback-fields.ts:32',
  },
  {
    table: 'chat_settings',
...
function main(): void {
  ...
  if (paths.length === 0) {
    throw new Error('usage: migrate-pascal-run-custom-columns.ts <fixture.db> [<fixture.db> …]');
  }
