 ...
 * sibling DB for the `doc_mount_files` repo from the committed plaintext spec
 * (`doc-mount-files-tier2.json`).
 *
 ...
 *   - seed through the REAL `DocMountFilesRepository`, whose overridden
 ...
 *
 * FRESH-FIXTURE NOTE: getCollection() runs generateDDL (emitting EXACTLY the 7
 * current schema columns) plus a non-UNIQUE sha256 index. The index is a read-path
 * accelerator only — it changes no persisted row bytes. There are NO runtime
 * ALTER-TABLE migrations on this repo.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'doc-mount-files-tier2.json');
  ...
  process.stderr.write(
    `built doc_mount_files mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
