 ...
 * sibling DB for the `group_character_members` repo from the committed plaintext
 * spec (`group-character-members-tier2.json`).
 *
 * ── THE SIBLING-DB MACHINERY (what's new here) ───────────────────────────────
 * Unlike every prior tier-2 builder, this repo's data lives in v4's dedicated
 ...
 *   - seed through the REAL `GroupCharacterMembersRepository`, whose overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'group-character-members-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-gcm-fixture-build-'));
  ...
  for (const row of spec.seed) {
    await repo.create(
      { groupId: row.groupId, characterId: row.characterId },
  ...
  process.stderr.write(
    `built group_character_members mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
