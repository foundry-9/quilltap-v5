 ...
 * `plugin_config` repo's `upsertForUserPlugin` case (Phase-2, MINTED-VALUES /
 * remap form) from the committed plaintext spec
 * (`plugin-config-upsert-tier2.json`).
 *
 * Same shape as build-plugin-config-fixture.ts: the `plugin_configs` table is
 ...
 * so the starting state is fully deterministic. Only the upsert ops applied
 * later mint their own (nondeterministic) ids/timestamps. One seed row OMITS
 * `enabled` (stored SQL NULL), exercising the optional-no-default boolean.
 *
 * Seed configs are constrained to {} or a SINGLE key (the open-JSON merge seam):
 * the upsert merges keep every stored config {}/single-key so v4's
 ...
 * applied later, by `cases/plugin-config-upsert-tier2.ts` (oracle) and the Rust
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'plugin-config-upsert-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-pc-upsert-fixture-build-'));
  ...
  process.stderr.write(
    `built plugin_configs upsert fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
