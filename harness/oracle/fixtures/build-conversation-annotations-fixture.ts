 ...
 * `conversation_annotations` repo (Phase-2) from the committed plaintext spec
 * (`conversation-annotations-tier2.json`).
 *
 ...
 * so the DDL (column set/order, the REAL-affinity `messageIndex`, the nullable
 * `sourceMessageId`) is identical to production by construction. Seed rows are
 * inserted via the real `ConversationAnnotationsRepository.create` with id +
 * timestamps pinned (CreateOptions), so the starting state is fully
 * deterministic. The two seed rows vary `messageIndex` (0 and 12) and exercise
 * both a null and a non-null `sourceMessageId`.
 *
 * The output file is the SEED-ONLY starting state. The op sequence under test is
 * applied later, by `cases/conversation-annotations-tier2.ts` (oracle) and the
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'conversation-annotations-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-ca-fixture-build-'));
  ...
  process.stderr.write(
    `built conversation_annotations fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
