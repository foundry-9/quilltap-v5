 ...
 * `help_docs` repo (Phase-2, repo #5) from the committed plaintext spec
 * (`help-docs-tier2.json`).
 *
 * Same shape as build-text-replacement-rules-fixture.ts: the `help_docs` table
 * is created by v4's OWN `ensureCollection('help_docs', HelpDocSchema)` so the
 * DDL (column set/order, the embedding BLOB column registration) is identical
 * to production by construction. Seed rows are inserted via the real
 * `HelpDocsRepository.create` with id + timestamps pinned (CreateOptions), so
 * the starting state is fully deterministic.
 *
 * THE BLOB POINT: the seed embedding is passed as a plain `number[]`; the real
 * `HelpDocsRepository` (which auto-registers the `embedding` blob column on its
 * first `getCollection()`) converts it to a little-endian Float32 BLOB on the
 * way in. A `null` embedding lands as SQL NULL. So BLOB serialization is done
 * entirely by v4's own code on the TS side — no hand-rolled buffer here.
 *
 * The output file is the SEED-ONLY starting state. The op sequence under test is
 * applied later, by `cases/help-docs-tier2.ts` (oracle) and the Rust harness,
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'help-docs-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-hd-fixture-build-'));
  ...
  process.stderr.write(
    `built help_docs fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
