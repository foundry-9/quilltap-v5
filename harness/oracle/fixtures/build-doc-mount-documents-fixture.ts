 ...
 * sibling DB for the `doc_mount_documents` repo from the committed plaintext spec
 * (`doc-mount-documents-tier2.json`).
 *
 * ── THE SIBLING-DB MACHINERY (mirrors doc_mount_points / doc_mount_folders) ───
 ...
 *   - seed through the REAL `DocMountDocumentsRepository`, whose overridden
 *     `getCollection()` creates the table in — and writes to — the mount-index DB
 *     (lazy `CREATE TABLE IF NOT EXISTS` + a UNIQUE index on fileId on first
 *     access; no explicit ensureCollection needed, unlike the main-DB builders).
 ...
 *
 * FRESH-FIXTURE NOTE: getCollection() creates the table from the CURRENT schema
 * via generateDDL, so the fixture has EXACTLY the 7 schema columns, in
 * schema/Zod field order, plus a UNIQUE index on fileId.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'doc-mount-documents-tier2.json');
  ...
  const { DocMountDocumentsRepository } = await import(
    '@/lib/database/repositories/doc-mount-documents.repository'
  );

  await initializeDatabase();

  // The repo's overridden getCollection() routes to the mount-index DB and
  // creates the table (+ UNIQUE fileId index) on first access — no explicit
  // ensureCollection needed.
  const repo = new DocMountDocumentsRepository();
  ...
  process.stderr.write(
    `built doc_mount_documents mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
