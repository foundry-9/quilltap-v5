 ...
 * `conversation_annotations` repo's `upsert` case (the minted-values / remap
 * form) from the committed plaintext spec
 * (`conversation-annotations-upsert-tier2.json`).
 *
 * Same shape as build-conversation-annotations-fixture.ts: the
 * `conversation_annotations` table is created by v4's OWN
 * `ensureCollection('conversation_annotations', ConversationAnnotationSchema)`
 * so the DDL (the REAL-affinity `messageIndex`, the nullable `sourceMessageId`)
 * is identical to production. Seed rows are inserted via the real
 * `ConversationAnnotationsRepository.create` with id + timestamps PINNED
 * (CreateOptions), so the pre-existing state is fully deterministic — only the
 * later `upsert` ops mint their own id/timestamps.
 *
 * The output file is the SEED-ONLY starting state. The upsert op sequence under
 * test is applied later, by `cases/conversation-annotations-upsert-tier2.ts`
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'conversation-annotations-upsert-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-ca-upsert-fixture-build-'));
  ...
  const { ConversationAnnotationSchema } = await import(
  ...
  process.stderr.write(
    `built conversation_annotations upsert fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
