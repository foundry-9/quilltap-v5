 ...
 * sibling DB for the `doc_mount_chunks` repo from the committed plaintext spec
 * (`doc-mount-chunks-tier2.json`).
 *
 ...
 *   - seed through the REAL `DocMountChunksRepository`, whose overridden
 *     `getCollection()` creates the table in — and writes to — the mount-index DB
 *     (lazy `CREATE TABLE IF NOT EXISTS` + two `CREATE INDEX IF NOT EXISTS` on
 *     first access; no explicit ensureCollection needed, unlike the main-DB
 *     builders). The indexes don't change column layout or stored cell bytes.
 ...
 * `DocMountChunksRepository` (which registers the `embedding` blob column on its
 * first `getCollection()`) converts it to a little-endian Float32 BLOB on the way
 * in. A `null` embedding lands as SQL NULL. So BLOB serialization is done
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'doc-mount-chunks-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-dmc-fixture-build-'));
  ...
  // creates the table (+ indexes) on first access — no explicit ensureCollection.
  ...
  process.stderr.write(
    `built doc_mount_chunks mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
