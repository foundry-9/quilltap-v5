/**
 * Fixture builder for the doc-edit ENUMERATION tool-handler differential
 * (doc_grep + doc_list_files — W4.1d batch 3b).
 *
 * Bakes the shared starting state across TWO databases (main + mount-index) so
 * BOTH v4 and the Rust port drive the same doc_grep / doc_list_files handlers
 * against a copy of byte-identical store state:
 ...
 *      v4's REAL repos.docMountFileLinks.linkDocumentContent (content-addressed by
 *      SHA-256, folders auto-created, markdown frontmatter policy derived). Gives
 *      grep/list a real file tree — multi-line hit files (for line numbers +
 *      context), a diacritics file, a subfolder, a `character_read:false` file
 *      (blocked), an auto-image path + a `.DS_Store` OS-cruft file (post-filter),
 *      and `.md`/`.txt` roots (for the `*.md` pattern filter).
 *
 * grep/list are READ-only, so the handlers' fire-and-forget Librarian/reindex/
 * embedding side effects never fire; the oracle mocks them harmless anyway.
 *
 * Run (Node 24, from the v4 checkout):
 *   N=~/.nvm/versions/node/v24.13.1/bin
 *   cd ~/source/quilltap-server
 *   QT_FIXTURE_DEN_MAIN=/tmp/qt-den-main.db QT_FIXTURE_DEN_MOUNT=/tmp/qt-den-mount.db \
 *     $N/npx tsx ~/source/quilltap-v5/harness/oracle/fixtures/build-doc-enum-fixture.ts
 */
...
async function main(): Promise<void> {
  ...
  const spec = JSON.parse(readFileSync(join(here, 'doc-enum.json'), 'utf8')) as Spec;
  const TS = spec.seedTimestamp;

  const mainOut = process.env.QT_FIXTURE_DEN_MAIN;
  const mountOut = process.env.QT_FIXTURE_DEN_MOUNT;
  if (!mainOut || !mountOut) {
    throw new Error('QT_FIXTURE_DEN_MAIN and QT_FIXTURE_DEN_MOUNT must both point at the .db files to write');
  }
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-den-fixture-build-'));
  ...
  //    from relativePath; parent folder rows auto-created; markdown frontmatter
  //    policy — e.g. character_read:false — derived at link time).
  ...
  process.stderr.write(
    `built doc-enum fixtures: main=${mainOut} mount=${mountOut} (V1=${v1}, OS1=${os1})\n`,
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`doc-enum fixture build failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
