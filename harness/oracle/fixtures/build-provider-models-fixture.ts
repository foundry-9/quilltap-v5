 ...
 * `provider_models` repo (Phase-2, repo #5) from the committed plaintext spec
 * (`provider-models-tier2.json`).
 *
 * Same shape as build-text-replacement-rules-fixture.ts: the `provider_models`
 * table is created by v4's OWN `ensureCollection('provider_models',
 * ProviderModelSchema)` so the DDL (column set/order, the REAL number columns,
 * the boolean columns) is identical to production by construction. Seed rows are
 * inserted via the real `ProviderModelsRepository.create` with id + timestamps
 * pinned (CreateOptions), so the starting state is fully deterministic. Every
 * column is supplied explicitly so no Zod default is exercised.
 *
 * The output file is the SEED-ONLY starting state. The op sequence under test is
 * applied later, by `cases/provider-models-tier2.ts` (oracle) and the Rust
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'provider-models-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-pm-fixture-build-'));
  ...
  process.stderr.write(
    `built provider_models fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
