 ...
 * sibling DB for the `doc_mount_folders` repo from the committed plaintext spec
 * (`doc-mount-folders-tier2.json`).
 *
 * ── THE SIBLING-DB MACHINERY ─────────────────────────────────────────────────
 * Like the pilot `group_character_members`, this repo's data lives in v4's
 * dedicated mount-index database (`quilltap-mount-index.db`), NOT the main DB.
 * v4 resolves that file from `SQLITE_MOUNT_INDEX_PATH` (or
 * `<dataDir>/quilltap-mount-index.db`), keyed with the SAME
 * `ENCRYPTION_MASTER_PEPPER`. So the recipe is:
 ...
 *   - seed through the REAL `DocMountFoldersRepository`, whose overridden
 ...
 *
 * NOTE: the seed's first folder is a ROOT (parentId = null); the second points
 * its parentId at the root — banking both the null and non-null nullable-UUID
 * paths. v4's getCollection lazily creates a UNIQUE INDEX on
 * (mountPointId, COALESCE(parentId,''), name); the seed names are distinct per
 * (mountPoint, parent) so no conflict arises.
 *
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'doc-mount-folders-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-dmf-fixture-build-'));
  ...
  const { DocMountFoldersRepository } = await import(
    '@/lib/database/repositories/doc-mount-folders.repository'
  );
  ...
  const repo = new DocMountFoldersRepository();
  ...
  process.stderr.write(
    `built doc_mount_folders mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
