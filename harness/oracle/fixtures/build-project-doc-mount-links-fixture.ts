 ...
 * sibling DB for the `project_doc_mount_links` repo from the committed plaintext
 * spec (`project-doc-mount-links-tier2.json`).
 *
 * ── THE SIBLING-DB MACHINERY (mirrors the group_character_members pilot) ──────
 * Like the pilot, this repo's data lives in v4's dedicated mount-index database
 ...
 *   - seed through the REAL `ProjectDocMountLinksRepository`, whose overridden
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'project-doc-mount-links-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-pdml-fixture-build-'));
  ...
  for (const row of spec.seed) {
    await repo.create(
      { projectId: row.projectId, mountPointId: row.mountPointId },
  ...
  process.stderr.write(
    `built project_doc_mount_links mount-index fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
