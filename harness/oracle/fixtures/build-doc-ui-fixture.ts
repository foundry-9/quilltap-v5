/**
 * Fixture builder for the doc-edit DOCUMENT-UI tool-handler differential
 * (W4.1d batch 3b — `doc_open_document` / `doc_close_document` / `doc_focus`).
 *
 ...
 *   4. A chat with the character as a CHARACTER participant, documentMode 'normal'
 *      initially (the schema default), so the open ops flip it to 'split' and the
 *      final close flips it back to 'normal'.
 *   5. Corpus files: a couple of .md files seeded into V1 via v4's REAL
 *      repos.docMountFileLinks.linkDocumentContent (the byte-landing path).
 *
 * The Librarian / reindex / embedding-scheduler side effects the runtime handlers
 * fire are documented no-op seams (mocked in the oracle case).
 *
 * Run (Node 24, from the v4 checkout):
 *   N=~/.nvm/versions/node/v24.13.1/bin
 *   cd ~/source/quilltap-server
 *   QT_FIXTURE_DUI_MAIN=/tmp/qt-dui-main.db QT_FIXTURE_DUI_MOUNT=/tmp/qt-dui-mount.db \
 *     $N/npx tsx ~/source/quilltap-v5/harness/oracle/fixtures/build-doc-ui-fixture.ts
 */
...
async function main(): Promise<void> {
  ...
  const spec = JSON.parse(readFileSync(join(here, 'doc-ui.json'), 'utf8')) as Spec;
  const TS = spec.seedTimestamp;

  const mainOut = process.env.QT_FIXTURE_DUI_MAIN;
  const mountOut = process.env.QT_FIXTURE_DUI_MOUNT;
  if (!mainOut || !mountOut) {
    throw new Error('QT_FIXTURE_DUI_MAIN and QT_FIXTURE_DUI_MOUNT must both point at the .db files to write');
  }
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-dui-fixture-build-'));
  ...
  // The doc-UI handlers write to chat_documents; v4 would create it lazily on
  // first repo access, but the Rust Writer opens the raw DB and needs the table
  // present. Materialize it in the fixture so both sides start with the schema.
  ...
  // 4. Chat with the character as a CHARACTER participant; documentMode 'normal'.
  await repos.chats.create(
    {
      userId: spec.userId,
      title: 'Doc UI Fixture',
      participants: [
        {
  ...
  // 5. Corpus files via the REAL linkDocumentContent.
  ...
  process.stderr.write(
    `built doc-ui fixtures: main=${mainOut} mount=${mountOut} (V1=${v1}, OS1=${os1})\n`,
  );
  process.exit(0);
}

main().catch((err) => {
  process.stderr.write(`doc-ui fixture build failed: ${err?.stack ?? err}\n`);
  process.exit(1);
});
