 ...
 * `prompt_templates` repo (Phase-2, repo #4) from the committed plaintext spec
 * (`prompt-templates-tier2.json`).
 *
 * Same shape as build-tags-fixture.ts: the `prompt_templates` table is created
 * by v4's OWN `ensureCollection('prompt_templates', PromptTemplateSchema)` so the
 * DDL (column set/order, the boolean + JSON-array column registration) is
 * identical to production by construction. Seed rows are inserted via the real
 * `PromptTemplatesRepository.create` with id + timestamps pinned (CreateOptions),
 * so the starting state is fully deterministic. One seed row is built-in
 * (`isBuiltIn: true`) so the op sequence can exercise the read-only guard; create
 * has no guard, so seeding it directly is fine.
 *
 * The output file is the SEED-ONLY starting state. The op sequence under test is
 * applied later, by `cases/prompt-templates-tier2.ts` (oracle) and the Rust
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'prompt-templates-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-prompt-templates-fixture-build-'));
  ...
  const { PromptTemplateSchema } = await import('@/lib/schemas/types');

  await initializeDatabase();
  await ensureCollection('prompt_templates', PromptTemplateSchema);
  ...
  process.stderr.write(
    `built prompt_templates fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
