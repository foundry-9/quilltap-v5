 ...
 * `character_plugin_data` repo (Phase-2) from the committed plaintext spec
 * (`character-plugin-data-tier2.json`).
 *
 * Same shape as build-image-profiles-fixture.ts: the `character_plugin_data`
 * table is created by v4's OWN `ensureCollection('character_plugin_data',
 * CharacterPluginDataSchema)` so the DDL (column set/order, the open-JSON `data`
 * column registration) is identical to production by construction. Seed rows are
 * inserted via the real `CharacterPluginDataRepository.create` with id +
 ...
 * applied later, by `cases/character-plugin-data-tier2.ts` (oracle) and the Rust
...
async function main(): Promise<void> {
  ...
  const specPath = join(here, 'character-plugin-data-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-cpd-fixture-build-'));
  ...
  process.stderr.write(
    `built character_plugin_data fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
