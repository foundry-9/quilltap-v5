 ...
 * `connection_profiles` repo (Phase-2, the workhorse profile repo) from the
 * committed plaintext spec (`connection-profiles-tier2.json`).
 *
 * Same shape as build-text-replacement-rules-fixture.ts: the
 * `connection_profiles` table is created by v4's OWN
 * `ensureCollection('connection_profiles', ConnectionProfileSchema)` so the DDL
 * (column set/order, the boolean + JSON column registration, the nullable-REAL
 * affinity) is identical to production by construction. Seed rows are inserted
 * via the real `ConnectionProfilesRepository.create` with id + timestamps pinned
 * (CreateOptions), so the starting state is fully deterministic. Every column is
 * supplied explicitly (no reliance on Zod defaults).
 *
 * The output file is the SEED-ONLY starting state. The op sequence under test is
 * applied later, by `cases/connection-profiles-tier2.ts` (oracle) and the Rust
...
interface Spec {
  ...
  seed: Array<Record<string, unknown> & { id: string; createdAt: string; updatedAt: string }>;
}

async function main(): Promise<void> {
  const here = dirname(fileURLToPath(import.meta.url));
  const specPath = join(here, 'connection-profiles-tier2.json');
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'qt-cp-fixture-build-'));
  ...
  process.stderr.write(
    `built connection_profiles fixture: ${out} (${spec.seed.length} seed rows)\n`
  );
  process.exit(0);
}
