...
function selectByFirstOptionText(
  fixture: ComponentFixture<CharacterDefaultsTab>,
  text: string,
): HTMLSelectElement {
...
describe('CharacterDefaultsTab (autosave contract)', () => {
  ...
  it('dispatches characterUpdate {defaultTimestampConfig: null} when the timestamp mode is Disabled', async () => {
    ...
    const fixture = await render(
      ...
      {
        character: character({
          defaultTimestampConfig: { mode: 'START_ONLY', format: 'FRIENDLY' },
        }),
      },
    );
