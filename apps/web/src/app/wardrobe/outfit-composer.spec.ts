...
function render(inputs: {
  ...
  slots: { top: string[]; bottom: string[]; footwear: string[]; accessories: string[] };
...
describe('OutfitComposer (v4 outfit-composer.tsx)', () => {
  ...
  it('renders bundle cards above the four slot rows, with the composite stripped from rows', () => {
    const fixture = render({
      items: [suit, jacket, pants],
      slots: { top: ['suit'], bottom: ['suit', 'pants'], footwear: [], accessories: [] },
    ...
    // …the four slot rows render (canonical order labels)…
    for (const label of ['Top', 'Bottom', 'Footwear', 'Accessories']) {
  ...
  it('hides bundle actions when showBundleActions=false (v4 `showActions`)', () => {
    const fixture = render({
      items: [suit, jacket, pants],
      slots: { top: ['suit'], bottom: ['suit'], footwear: [], accessories: [] },
