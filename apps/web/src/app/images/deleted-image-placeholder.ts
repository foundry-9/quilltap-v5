...
@Component({
  ...
  template: `
    ...
    >
      ...
      <button
        type="button"
        class="qt-bg-destructive hover:qt-bg-destructive/90 qt-text-on-destructive rounded transition-colors {{ isCompact() ? 'px-1.5 py-0.5 text-[9px]' : 'px-3 py-1 text-xs' }}"
        (click)="handleCleanup()"
      >
        Remove
      </button>
